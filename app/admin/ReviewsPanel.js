...
export default function ReviewsPanel() {
  ...
  if (loading) {
    return <div style={{ padding: 24, textAlign: 'center', color: C.gray }}>Loading…</div>
  }

  if (forbidden) {
    return (
      <div style={{ background: '#fff', borderRadius: 10, padding: 32, textAlign: 'center', color: '#9CA3AF', fontSize: 13 }}>
        The Reviews tab is restricted to content_approver accounts and above.
      </div>
    )
  }
...
function ReviewDetail({ attemptId, onBack, onSubmitted, onError }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <button onClick={onBack} style={{ background: 'none', border: 'none', color: C.gold, fontSize: 13, fontWeight: 600, cursor: 'pointer', padding: 0, textAlign: 'left' }}>← Back to queue</button>
