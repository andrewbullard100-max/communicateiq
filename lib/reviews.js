...
export async function listReviewQueue(orgId, userId, limit = 50) {
...
export async function getAttemptForReview(orgId, attemptId, userId) {
  const db = getSupabaseScoped(orgId, userId)
  ...
  const transcript = await getTranscript(attemptId, orgId, userId)
...
export async function submitHumanReview({ orgId, attemptId, reviewerId, scores, notes }) {
  // reviewerId is the signed-in caller here (see app/api/admin/reviews/[attemptId]/route.js),
  // so it doubles as the userId for RLS scoping.
  const db = getSupabaseScoped(orgId, reviewerId)
  await assertAttemptInOrg(db, attemptId, orgId)
  ...
  await markReviewedAndPurgeIfDue(attemptId, orgId, reviewerId) // honors 'until_manager_review' retention — see lib/transcripts.js
  return { ok: true }
}
...
export async function getAgreementStats(orgId, userId) {
  const db = getSupabaseScoped(orgId, userId)
