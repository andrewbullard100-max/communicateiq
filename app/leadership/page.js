...
const TOPIC_LABELS = {
  'raw-material-cost-control': 'Raw Material Cost',
...
function buildSystemPrompt(scenario, pressureLevel, adaptiveDirective) {
  ...
  return `You are an AI-powered leadership training evaluator for CommunicateIQ Leadership & Operations Training.

You play TWO roles simultaneously:

ROLE 1 — COUNTERPART CHARACTER
${scenario.counterpartPersona}

Start with this OPENING LINE: "${scenario.openingLine}"

Stay fully in character. Push back when the trainee is vague, dismissive, or avoids the real issue. Do NOT break character. Keep responses to 2-4 sentences — this is a voice conversation.
...
SIMULATION_COMPLETE
{
  ...
  "certificationStatus": "<Certified — Field Ready | Conditional — Coaching Required | Not Yet Certified — Development Required>"
}

RUBRIC: 1=Weak 2=Developing 3=Proficient 4=Distinguished

LEADERSHIP CONTEXT: Unlike client-facing scenarios, the trainee here holds real authority over the counterpart (a direct report, supervisor, or team) or is negotiating as a peer with a vendor. Score them on how they use that authority — coaching vs. blaming, holding people accountable vs. avoiding the hard conversation, making a fast decisive call under pressure vs. freezing, negotiating firmly but fairly. The specific dynamics for this scenario are detailed in the COUNTERPART CHARACTER persona above — follow those exactly.
...
function persistResult(scenario, scored, assignmentId, pressureLevel) {
...
function LeadershipPage() {
  ...
  const [scenarios, setScenarios] = useState(LEADERSHIP_SCENARIOS) // instant first paint; replaced below if the DB fetch succeeds
  const [scenarioSource, setScenarioSource] = useState('local')

  // This is a standalone track — default to whatever industry the person
  // already picked elsewhere, but let them switch freely on this page.
  useEffect(() => {
    if (typeof window === 'undefined') return
    const saved = sessionStorage.getItem('selectedIndustry')
    if (saved) setIndustryId(saved)
  }, [])

  // Prefer live content from Supabase; keep the bundled fallback already in
  // state if the fetch fails.
  useEffect(() => {
    fetch('/api/scenarios?set=leadership')
      .then(res => res.json())
      .then(data => {
  ...
  const filteredScenarios = scenarios.filter(s =>
    s.industry === industryId && (topicFilter === 'all' || s.trainingType === topicFilter)
  )
  ...
  // Adaptive difficulty: same as Simulation — pull attempt history to
  // recommend a starting pressure level and silently target a demonstrated
  // weak dimension this rep (see lib/pressure.js).
  useEffect(() => {
  ...
  function startSilenceDetection(stream, onSilence) {
    ...
    function check() {
      ...
      if (speechAvg < 25) {
        ...
        else if (Date.now() - silenceStart > SILENCE_MS) { onSilence(); return }
      } else {
        silenceStart = null
      }
  ...
  async function handleSend(userText) {
    ...
    try {
      ...
      if (scored) {
        setResult(scored)
        persistResult(selectedRef.current, scored, assignmentId, pressureLevelRef.current)
        setScreen('results')
      } else {
  ...
  async function endAndScore() {
    ...
    try {
      ...
      if (scored) {
        ...
        if (scoredRetry) { setResult(scoredRetry); persistResult(selectedRef.current, scoredRetry, assignmentId, pressureLevelRef.current); setScreen('results') }
      }
    } catch {}
    isLoading.current = false
    setLoading(false)
    ...
    <div style={{ minHeight: '100vh', background: '#F4F6F9' }}>
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '36px 24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 32 }}>
          <Link href="/dashboard" className="btn-ghost" style={{ fontSize: 12, padding: '8px 14px' }}>Back to Platform</Link>
          <span style={{ color: '#6B7280', fontSize: 12 }}>Optional Track · Leadership & Operations</span>
        </div>
        <span className="label">Managing Labor, Cost, and People</span>
        <h1 className="section-title fade-up" style={{ marginBottom: 8 }}>Leadership & Operations Scenarios</h1>
        ...
        <p style={{ color: '#6B7280', fontSize: 14, marginBottom: 24 }}>
          These scenarios are different from Role-Play Simulations — instead of talking to a client, you're managing your own team: coaching, holding people accountable, negotiating with vendors, and leading through labor and cost pressure.
        </p>

        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
          {INDUSTRIES.map(i => (
            <button key={i.id} onClick={() => setIndustryId(i.id)}
              style={{
                background: industryId === i.id ? '#1C2B5E' : '#FFFFFF',
                color: industryId === i.id ? '#FFFFFF' : '#374151',
                border: `1.5px solid ${industryId === i.id ? '#1C2B5E' : '#D1D5DB'}`,
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 28 }}>
          {[{ id: 'all', label: 'All Topics' }, ...Object.entries(TOPIC_LABELS).map(([id, label]) => ({ id, label }))].map(t => (
            <button key={t.id} onClick={() => setTopicFilter(t.id)}
              style={{
                background: topicFilter === t.id ? '#0D9488' : 'transparent',
                color: topicFilter === t.id ? '#FFFFFF' : '#6B7280',
                border: `1px solid ${topicFilter === t.id ? '#0D9488' : '#D1D5DB'}`,
              ...
              <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', gap: 8, marginBottom: 4, flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    <span style={{ fontSize: 10, color: '#6B7280' }}>{TOPIC_LABELS[s.trainingType]}</span>
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '36px 24px' }}>
        ...
        <div className="fade-up-2" style={{ marginBottom: 14, background: 'rgba(28,43,94,0.04)', border: '1.5px solid #1C2B5E', borderLeft: '4px solid #0D9488', borderRadius: 8, padding: '14px 18px' }}>
          ...
          <p style={{ fontSize: 13, color: '#374151', lineHeight: 1.7, margin: 0 }}>
            Unlike a client conversation, you hold real authority here. Part of what this simulation measures is whether you use that authority well — coaching instead of blaming, holding people accountable instead of avoiding the hard conversation, and making fast, clear decisions under pressure.
          </p>
        </div>
        ...
        <div className="card fade-up-4" style={{ marginBottom: 14, borderColor: '#1C2B5E', background: 'rgba(28,43,94,0.04)' }}>
          <span className="label">Opening Line — Delivered by AI Counterpart</span>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ fontSize: 14, color: '#374151', lineHeight: 1.7, marginBottom: 8 }}>
              Take a moment to review the scenario data above.
            </p>
            <p style={{ fontSize: 13, color: '#6B7280', lineHeight: 1.6, marginBottom: 32 }}>
              When you click <strong style={{ color: '#1C2B5E' }}>Begin Simulation</strong>, the AI counterpart will deliver their opening line and your microphone will open automatically after they finish speaking.
            </p>
            <button className="btn-primary" style={{ fontSize: 15, padding: '14px 36px', letterSpacing: 0.5 }} onClick={launchSimWithGesture}>
              Begin Simulation →
            </button>
          </div>
        </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '24px 20px', maxWidth: 780, width: '100%', margin: '0 auto' }}>
        {speaking && (
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 4, padding: '16px 0' }}>
            ...
            <span style={{ marginLeft: 12, fontSize: 12, color: '#6B7280' }}>AI Counterpart Speaking</span>
          </div>
        )}
          ...
          <div key={i} style={{ display: 'flex', flexDirection: msg.role === 'user' ? 'row-reverse' : 'row', gap: 10, marginBottom: 16 }}>
            ...
            <div style={{ background: msg.role === 'user' ? '#1C2B5E' : '#FFFFFF', border: `1.5px solid ${msg.role === 'user' ? '#1C2B5E' : '#D1D5DB'}`, borderRadius: 10, padding: '12px 16px', maxWidth: '80%', borderTopRightRadius: msg.role === 'user' ? 2 : 10, borderTopLeftRadius: msg.role === 'assistant' ? 2 : 10, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
              <div style={{ fontSize: 10, color: msg.role === 'assistant' ? '#0D9488' : 'rgba(255,255,255,0.6)', fontWeight: 700, letterSpacing: 1, marginBottom: 6 }}>
                {msg.role === 'assistant' ? 'AI COUNTERPART' : 'YOU — GM'}
              </div>
  ...
  if (screen === 'results' && result) {
    ...
    const statusColor = result.certificationStatus.includes('Certified') ? C.green : result.certificationStatus.includes('Conditional') ? '#b87333' : C.red
    return (
      <div style={{ minHeight: '100vh', background: '#F4F6F9' }}>
        <div style={{ maxWidth: 760, margin: '0 auto', padding: '36px 24px' }}>
          <span className="label fade-up">CommunicateIQ · Leadership Assessment Results</span>
