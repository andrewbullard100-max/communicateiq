...
export async function listOrgUsers(orgId, userId) {
...
export async function updateOrgUser(orgId, userId, { name, status, managerId, orgUnitId, roles }, callerId) {
  const db = getSupabaseScoped(orgId, callerId)
...
export async function deactivateOrgUser(orgId, userId, callerId) {
  return updateOrgUser(orgId, userId, { status: 'terminated' }, callerId)
}

export async function resetOrgUserPassword(orgId, userId, newPassword, callerId) {
  const db = getSupabaseScoped(orgId, callerId)
  await assertUserInOrg(db, userId, orgId)
...
export async function listRecentLoginEvents(orgId, userId, limit = 50) {
  const db = getSupabaseScoped(orgId, userId)
