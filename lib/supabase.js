...
//
// getSupabaseScoped(orgId, userId) — the anon/publishable key, with the
// caller's org id (and, where available, user id) sent as custom request
// headers on every call. RLS policies (see migrations 001-003) read
// x-app-org-id via current_org_id() to scope every query to that org at the
// database level — this is what makes tenant isolation real instead of
// "every app-code query remembered to filter by org_id correctly."
// Application code should still filter by org_id/user_id explicitly too
// (see individual lib/*.js files) — that's defense in depth, not a
// substitute for RLS, but RLS is now the actual backstop.
//
// x-app-user-id feeds current_user_id(), which the manager/org-unit
// visibility policies (see migration add_manager_org_unit_visibility_scope)
// depend on to know who's asking — without it those policies can't tell a
// manager from a stranger and, being restrictive, resolve to "show nothing"
// on the tables they cover (users, simulation_attempts, transcripts,
// score_reviews). userId is optional only because a few call sites
// (assertSeatAvailable-style lookups made before a session exists) have no
// user to scope by; any call that reads from those four tables on behalf of
// a signed-in caller should always pass it.
//
...
export function getSupabaseScoped(orgId, userId) {
  const url = process.env.SUPABASE_URL
  ...
  const headers = { 'x-app-org-id': orgId }
