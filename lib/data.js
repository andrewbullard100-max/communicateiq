...
export const TRAINING_TYPES = {
  'higher-ed': [
    { id: 'executive-communication', label: 'Executive & Campus Dining Communication', icon: '🎤', desc: 'CFO, VP, and campus stakeholder conversations' },
    { id: 'facilities-maintenance', label: 'Facilities — Maintenance', icon: '🔧', desc: 'Campus operations & maintenance stakeholder conversations' },
    { id: 'facilities-housekeeping', label: 'Facilities — Housekeeping', icon: '🧹', desc: 'Housing operations & housekeeping stakeholder conversations' },
  ],
  'senior-living': [
    { id: 'family-resident-relations', label: 'Family & Resident Relations', icon: '💬', desc: 'Family council, resident advocacy & survey readiness' },
    { id: 'facilities-maintenance', label: 'Facilities — Maintenance', icon: '🔧', desc: 'Executive Director & resident-safety facilities conversations' },
    { id: 'facilities-housekeeping', label: 'Facilities — Housekeeping', icon: '🧹', desc: 'Infection-control & housekeeping stakeholder conversations' },
  ],
  'acute-care': [
    ...
    { id: 'facilities-maintenance', label: 'Facilities — Maintenance', icon: '🔧', desc: 'Plant operations & surgical services facilities conversations' },
    { id: 'facilities-housekeeping', label: 'Facilities — Housekeeping', icon: '🧹', desc: 'Infection prevention & patient flow housekeeping conversations' },
  ],
  'corporate-dining': [
    ...
    { id: 'facilities-maintenance', label: 'Facilities — Maintenance', icon: '🔧', desc: 'Property owner & facilities lead conversations' },
    { id: 'facilities-housekeeping', label: 'Facilities — Housekeeping', icon: '🧹', desc: 'Executive-space & event housekeeping conversations' },
  ],
  'k12': [
    { id: 'district-family-relations', label: 'District & Family Relations', icon: '🍎', desc: 'Superintendent, principal & parent conversations' },
    { id: 'facilities-maintenance', label: 'Facilities — Maintenance', icon: '🔧', desc: 'Superintendent-facing facilities & emergency repair conversations' },
    { id: 'facilities-housekeeping', label: 'Facilities — Housekeeping', icon: '🧹', desc: 'Public-facing cleanliness & housekeeping conversations' },
  ],
}

// ─── Modules ──────────────────────────────────────────────────────────────────
// CORE_FLOW is the ordered certification path, rendered as a guided stepper on
// the home page (see app/page.js). SECONDARY_MODULES are optional
// utilities shown separately, not part of the numbered flow. Role-restricted
// areas (Team, Reviews, Policies, Admin) live as tabs inside the Admin
// Console (app/admin/page.js) rather than as home-page tiles — see
// lib/auth.js's role constants for who can see which tab.
export const CORE_FLOW = [
  ...
  { id: 'stakeholder', href: '/stakeholder', icon: '🗺️', label: 'Stakeholder Mapping',     day: 'Foundations',        desc: 'Build your account map with AI coaching' },
  { id: 'simulation',  href: '/simulation',  icon: '🎭', label: 'Role-Play Simulations',   day: 'Practice',           desc: 'Live AI client conversations with rubric scoring' },
  { id: 'financial',   href: '/financial',   icon: '📊', label: 'Financial Storytelling',  day: 'Financial Fluency',  desc: 'Translate P&L into leadership language' },
  { id: 'qbr',         href: '/qbr',         icon: '🏛️', label: 'QBR Builder & Delivery',  day: 'Executive Delivery', desc: 'Build and present a QBR to an AI boardroom' },
  { id: 'dashboard',   href: '/dashboard',   icon: '📈', label: 'Certification Dashboard', day: 'Certification',      desc: 'Track progress and certification status' },
]
...
// Retained for any code still importing the old flat list (e.g. search
// tooling) — prefer CORE_FLOW / SECONDARY_MODULES for new UI.
export const MODULES = [...CORE_FLOW, ...SECONDARY_MODULES]
// ─── Scenarios ────────────────────────────────────────────────────────────────
export const SCENARIOS = [
  {
    id: 'D1-S1', day: 'Scenario 1', title: 'CFO Calls After Orientation Complaint',
...
IMPORTANT: The GM is a third-party contractor. They cannot cancel university events, cannot enforce consequences on university staff, and cannot hold university personnel accountable. If the GM proposes any of these actions, push back: "You can't cancel our events — that's not your call to make." The correct GM actions are: document the communication gap, escalate internally through the dining provider's chain, create a written protocol agreement, and build a paper trail.`,
  ...
  {
    id: 'D1-S2', day: 'Scenario 2', title: 'Survey Drop — VP of Student Life',
...
If the GM is vague, push: "What are you actually going to do differently, and by when?" Push for specifics and timelines.`,
  ...
  {
    id: 'D2-S1', day: 'Scenario 5', title: 'Client Questions Rising Costs and Service Request',
    ...
    dataPacket: {
      title: 'Data Packet — Client-Facing Summary',
...
If they make a clean case for the scope expansion, ask: "What are my options before I decide?"`,
  ...
  {
    id: 'D3-S2', day: 'Scenario 6', title: 'Donor Dinner Failure — Service Recovery',
...
If they lead with clear ownership and a specific plan, soften slightly but still ask hard questions about what has actually changed.`,
  ...
  {
    id: 'D1-S3', day: 'Scenario 3', title: 'Student Complaint — Undercooked Chicken',
...
- Ask at least once: "Should I go to the health center?"`,
  ...
  {
    id: 'D1-S4', day: 'Scenario 4', title: 'Meal Plan Exception — Student and Parent',
...
- The student speaks once, quietly: "I just do not want to feel like I cannot eat anything"`,
  ...
  {
    id: 'D3-S3', day: 'Scenario 7', title: 'Sysco Director — Delivery Shortages',
...
// ═══════════════════════════ FACILITIES — MAINTENANCE (Client) ═══════════════════════════
  {
    id: 'HE-FM-S1', day: 'Facilities', title: 'Deferred Maintenance Backlog Surfaces on a Walkthrough',
    focus: 'Six-Part Framework under scrutiny', difficulty: 'Foundational', icon: '🔧',
    industry: 'higher-ed', trainingType: 'facilities-maintenance',
    context: 'The VP of Campus Operations is doing a quarterly facilities walkthrough with you. She stops at three open work orders — a leaking roof drain, a broken exterior door closer, and a flickering hallway light — all open more than 45 days. She wants to know why.',
    dataPacket: {
      title: 'Open Work Orders — Building 200, Age > 30 Days',
      headers: ['Work Order', 'Opened', 'Age', 'Status', 'Reason for Delay'],
      rows: [
        ['Roof drain leak — 2nd floor', '58 days ago', '58 days', 'Awaiting parts', 'Custom flashing on backorder'],
        ['Exterior door closer — Bldg 200 main', '51 days ago', '51 days', 'Scheduled', 'Lower priority than life-safety items'],
        ['Hallway light ballast', '46 days ago', '46 days', 'Scheduled', 'Batched with electrical rounds'],
      ],
    },
    openingLine: "These have been open over a month and a half. I walk this building every week — how am I only hearing about this now?",
    clientPersona: `VP of Campus Operations — data-driven, not looking to punish, but she owns facilities condition reporting to the Board and doesn't like surprises on her own walkthroughs.

What she actually cares about:
- Whether there's a real, working prioritization system behind the backlog — not chaos
- Being told about delays proactively, not finding them herself
- Whether life-safety and building-envelope items are triaged ahead of cosmetic ones

What she does NOT ask about:
- Your internal labor cost or technician headcount planning — not her concern
- Your vendor markup on parts — irrelevant to her

If the GM is vague about why these specific items were deprioritized, she presses: "Walk me through your triage logic — why is a door closer scheduled but a roof leak isn't fixed yet?"
If the GM proposes something outside their authority (e.g., promising a capital replacement budget that isn't theirs to allocate), she corrects them: "That's a capital request — that goes through my office, not yours to just decide."`,
    successCriteria: [
      'Explains the actual triage logic (life-safety/envelope first) rather than apologizing without substance',
      'Distinguishes the parts-backorder delay (outside their control) from the scheduling delay (within their control)',
      'Commits to a proactive reporting cadence so this doesn\'t surface on a walkthrough again',
      'Does not overpromise capital funding that isn\'t within their authority',
      'Gives specific dates for each open item, not "soon"',
      'Tone is calm and accountable, not defensive',
    ],
  },
  {
    id: 'HE-FM-S2', day: 'Facilities', title: 'HVAC Failure in a Dorm During Move-In Week',
    focus: 'Crisis communication under time pressure', difficulty: 'Advanced', icon: '🌡️',
    industry: 'higher-ed', trainingType: 'facilities-maintenance',
    context: "The chiller serving a 400-bed residence hall failed overnight, the day before move-in. Temperatures in the building are climbing. Parents are already posting on the class Facebook group. The Director of Residence Life is calling you for a plan, not a postmortem.",
    dataPacket: null,
    openingLine: "I have four hundred families arriving tomorrow and a building with no air conditioning. What is the actual plan, right now?",
    clientPersona: `Director of Residence Life — under intense pressure herself from parents and the Dean of Students. Not interested in root-cause analysis right now; wants a concrete plan for the next 24 hours.

What she actually cares about:
- What's being done RIGHT NOW to make move-in tolerable (fans, portable units, revised schedule)
- A specific, credible timeline for the actual repair
- What she should tell parents who are already asking

What she does NOT ask about:
- The technical root cause of the chiller failure — she'll ask later, not now
- Your vendor relationship or parts costs

If the GM launches into technical explanation before addressing the immediate plan, she cuts them off: "I don't need the mechanical explanation right now — what are we doing for these students tonight?"
If the GM gives a vague repair timeline, she pushes: "'A few days' isn't something I can tell four hundred families. Give me a real number."`,
    successCriteria: [
      'Leads with the immediate mitigation plan (portable cooling, fans, revised move-in flow) before any root-cause detail',
      'Gives a specific, credible repair timeline rather than a vague one',
      'Offers language she can actually use with parents',
      'Does not over-promise a repair time they can\'t guarantee',
      'Proposes a specific check-in time to update her, not "I\'ll keep you posted"',
      'Stays calm and solution-first despite the pressure in her tone',
    ],
  },
  {
    id: 'HE-FM-S3', day: 'Facilities', title: 'Fire Alarm Pulled at 2 AM in a Full Residence Hall',
    focus: 'Crisis communication under time pressure', difficulty: 'Advanced', icon: '🚨',
    industry: 'higher-ed', trainingType: 'facilities-maintenance',
    context: "A fire alarm went off at 2:10 AM in a 600-bed residence hall — a false alarm triggered by a malfunctioning smoke detector on the 4th floor, not an actual fire. Students evacuated into the cold for 40 minutes while your on-call technician traced the fault and reset the panel. This is the third false alarm in that building this semester. The Director of Residence Life is calling you first thing in the morning, before the 8 AM meeting where she has to brief the Dean of Students.",
    dataPacket: {
      title: 'Fire Alarm Panel Activity — Building 400, This Semester',
      headers: ['Date', 'Trigger', 'Time to Reset', 'Cause'],
      rows: [
        ['Sept 3', 'Smoke detector, 4th floor', '35 min', 'Dust buildup in detector head'],
        ['Oct 12', 'Smoke detector, 2nd floor', '28 min', 'Detector near end of service life'],
        ['Last night', 'Smoke detector, 4th floor', '40 min', 'Same detector as Sept 3 — replacement was scheduled but not completed'],
      ],
    },
    openingLine: "Six hundred students stood outside in the cold at two in the morning for a detector we already knew was bad. How does that happen a second time?",
    clientPersona: `Director of Residence Life — exhausted from being woken up at 2 AM, and now has to explain this to the Dean in a few hours. Not interested in excuses about parts lead times; wants to know why a known-bad detector wasn't already replaced.

What she actually cares about:
- Why the September detector wasn't actually replaced after it was flagged
- What specifically is different this time so it doesn't happen a third time
- What she can tell the Dean at 8 AM and what she can tell students and parents if this becomes a bigger story

What she does NOT ask about:
- The technical wiring detail of how the panel resets — not her concern
- Your after-hours on-call staffing costs — irrelevant to her right now

If the GM's answer to "why wasn't it replaced" is vague or blame-shifting, she presses: "You told me in September this was handled. Was it handled or wasn't it?"
If the GM proposes a plan without a specific date, she pushes: "Give me an actual day this gets fixed, not 'soon' — I have to tell the Dean something concrete."`,
    successCriteria: [
      'Owns the miss directly — acknowledges the September detector should have been replaced and explains honestly why it wasn\'t (parts, scheduling, or tracking failure)',
      'Does not minimize the impact of 600 students standing outside at 2 AM',
      'Gives a specific, same-day or next-day plan to replace the detector, not a vague timeline',
      'Proposes a system fix (e.g., a tracking check to close the loop on flagged detectors) so this doesn\'t become a fourth incident',
      'Gives her language she can use with the Dean and, if needed, with students and parents',
      'Stays calm and accountable despite her frustration',
    ],
  },
  {
    id: 'HE-FM-S4', day: 'Facilities', title: 'Late-Night Water Leak Threatens a Library Archive',
    focus: 'Crisis communication under time pressure', difficulty: 'Advanced', icon: '💧',
    industry: 'higher-ed', trainingType: 'facilities-maintenance',
    context: "At 11:40 PM your on-call technician found water actively dripping from a ceiling tile directly above the special collections archive room in the library — a burst supply line one floor up. Your team has shut the water and is setting up containment, but some materials may already be at risk. The Dean of Libraries, who oversees the archive, is on the phone wanting to know what's happening before she drives in.",
    dataPacket: null,
    openingLine: "You're telling me water is dripping onto the archive room and you found out over an hour ago. What exactly has been done, and do I need to come in right now?",
    clientPersona: `Dean of Libraries — calm but very focused, because some of the materials in that room are irreplaceable. She will drive in regardless, but wants to know exactly what's been done before she does.

What she actually cares about:
- Whether the water source is actually stopped, not just that someone is "working on it"
- Whether anything in the archive room has been protected or needs to be moved right now
- A clear picture of what she'll find when she arrives, so she isn't walking in blind

What she does NOT ask about:
- The plumbing repair timeline for the burst line itself — that's secondary to her right now
- Your after-hours labor costs — not a priority in this moment

If the GM is vague about whether the water is actually stopped, she presses: "Is the water off or not? I need a yes or no before I get in my car."
If the GM hasn't already moved to protect materials in the room, she pushes: "Has anyone actually gone in and covered or moved anything, or is it just sitting under a drip right now?"`,
    successCriteria: [
      'Confirms clearly and specifically that the water source is shut off, not just "being worked on"',
      'States plainly what protective action has or hasn\'t been taken in the archive room itself (covering materials, moving at-risk items, containment)',
      'Is honest if something has not yet been done, rather than implying more progress than is real',
      'Gives her a clear picture of what she\'ll see on arrival so she isn\'t walking in blind',
      'Proposes next steps (e.g., a conservator or collections specialist being called) rather than treating this as purely a facilities repair',
      'Stays calm and precise — this is a high-stakes, low-margin-for-error conversation',
    ],
  },
  {
    id: 'HE-FM-S5', day: 'Facilities', title: 'Planned Power Shutoff Will Disrupt Classes and a Residence Hall',
    focus: 'Proactive stakeholder communication', difficulty: 'Advanced', icon: '⚡',
    industry: 'higher-ed', trainingType: 'facilities-maintenance',
    context: "An electrical switchgear inspection has found a failing component that must be replaced before it fails unpredictably. The only safe way to do the replacement is a planned power shutoff to a section of campus that includes two academic buildings with classes in session and one residence hall. You need to propose timing and get sign-off from the VP of Campus Operations before you can notify anyone else.",
    dataPacket: {
      title: 'Affected Buildings — Proposed Shutoff Window',
      headers: ['Building', 'Impact', 'Scheduling Conflict'],
      rows: [
        ['Hargrove Hall', '14 classes in session during a weekday window', 'Three sections have exams scheduled this week'],
        ['Whitfield Science Center', 'Labs with temperature-sensitive samples', 'Backup generator covers critical refrigeration only'],
        ['Copeland Residence Hall', '310 residents lose power, elevators, and hot water', 'No safe daytime alternative — HVAC and security systems also affected'],
      ],
    },
    openingLine: "You want to cut power to a residence hall and two academic buildings. Convince me this can't wait, and convince me you've actually thought through who gets hurt by the timing.",
    clientPersona: `VP of Campus Operations — supportive of proactive safety work in principle, but will not approve a plan that creates avoidable chaos. She has to answer to the Provost's office and Residence Life if this goes badly.

What she actually cares about:
- Whether this genuinely can't be deferred, or is being framed as more urgent than it is
- Whether the proposed timing actually minimizes disruption (exam schedules, lab needs, residents)
- What communication plan exists for the people affected, and how far in advance they'll know

What she does NOT ask about:
- The technical specifics of the switchgear component — she trusts the safety judgment call, she's testing the planning
- Vendor selection for the replacement part — not her concern

If the GM proposes a shutoff window without checking the academic or residential calendar, she pushes back hard: "Did you actually check if there are exams that week, or are you guessing?"
If the GM is vague on communication timing, she presses: "How many days of notice do residents get, and who is telling them — you or me?"`,
    successCriteria: [
      'Clearly explains why the shutoff cannot be safely deferred, in terms she can repeat to the Provost',
      'Shows the timing was chosen around the academic and residential calendar, not just crew availability',
      'Proposes a specific notice period and communication plan for affected students, faculty, and residents',
      'Addresses the residence hall impact specifically (hot water, elevators, security systems) with a mitigation plan, not just an apology',
      'Proposes contingencies for the lab refrigeration risk rather than treating it as someone else\'s problem',
      'Asks for her sign-off on a specific date/window rather than leaving it open-ended',
    ],
  },
  {

    id: 'AC-FM-S1', day: 'Facilities', title: 'Life-Safety Finding During a Joint Commission Survey',
    focus: 'Regulatory accountability under scrutiny', difficulty: 'Advanced', icon: '🚨',
    industry: 'acute-care', trainingType: 'facilities-maintenance',
    context: "A Joint Commission surveyor just flagged a fire door in the surgical wing that isn't latching properly, and found the emergency generator's monthly load test log incomplete for two months. The Director of Plant Operations for the hospital wants an explanation before the surveyor's exit conference this afternoon.",
    dataPacket: {
      title: 'Findings Summary',
      headers: ['Finding', 'Requirement', 'Status', 'Risk Level'],
      rows: [
        ['Fire door — surgical wing corridor', 'Must self-latch, NFPA 80', 'Not latching — hardware worn', 'High — cited'],
        ['Generator load test log', 'Monthly test, documented', 'Missing entries — 2 months', 'High — cited'],
      ],
    },
    openingLine: "The surveyor is going to cite us on both of these in about three hours. I need to know exactly what happened and what you're doing before that exit conference.",
    clientPersona: `Director of Plant Operations for the hospital — this is her license and her hospital's accreditation on the line, not an abstract compliance issue. Direct, technical, and under real time pressure.

What she actually cares about:
- Immediate corrective action she can report to the surveyor before exit conference
- Whether this is a one-off gap or a systemic documentation failure — because that changes what she has to tell her CEO
- A specific written corrective action plan, since Joint Commission will require one regardless

What she does NOT tolerate:
- Excuses or minimizing — "it's just paperwork" is the wrong answer for a load test log
- Vague timelines — she needs dates for the plan of correction

If the GM downplays the missing log as "just paperwork," she pushes back hard: "That log is how we prove the generator will run when a patient's on a vent. It is not just paperwork."`,
    successCriteria: [
      'Takes immediate, specific corrective action on the fire door (work order, timeline) rather than defending it',
      'Acknowledges the log gap directly without minimizing its patient-safety significance',
      'Distinguishes whether this is isolated or systemic — and is honest either way',
      'Commits to a written plan of correction with real dates, matching what Joint Commission will require',
      'Does not get defensive under the surveyor-driven time pressure',
      'Proposes a preventive fix (e.g., a redundant log-completion check) not just closing the immediate finding',
    ],
  },
  {
    id: 'AC-FM-S2', day: 'Facilities', title: 'Capital Equipment Failure Disrupts the OR Schedule',
    focus: 'Executive communication under operational pressure', difficulty: 'Advanced', icon: '⚙️',
    industry: 'acute-care', trainingType: 'facilities-maintenance',
    context: "A steam sterilizer serving Central Sterile failed this morning, forcing three OR cases to be rescheduled. The VP of Surgical Services wants to understand what happened and whether it will happen again.",
    dataPacket: null,
    openingLine: "Three surgeons had to reschedule cases this morning because of a sterilizer failure. That is a very expensive morning. What happened, and how do I know it won't happen again next week?",
    clientPersona: `VP of Surgical Services — thinks in terms of OR utilization and surgeon relationships, both of which are damaged by a morning like this. Not a facilities expert but sophisticated about operational impact.

What he actually cares about:
- The financial and scheduling impact in terms he tracks (canceled case revenue, surgeon trust)
- Whether this was predictable (missed PM) or a genuine unexpected failure
- Concrete assurance about recurrence risk — backup capacity, PM schedule changes

What he does NOT ask about:
- The specific mechanical failure mode in technical detail
- Internal maintenance staffing levels or budget

If the GM can't clearly say whether this was a preventable PM miss or a true unexpected failure, he presses: "I need to know if this was foreseeable, because that changes what I tell the surgeons."`,
    successCriteria: [
      'States plainly whether the failure was linked to preventive maintenance or genuinely unforeseeable',
      'Quantifies impact in the client\'s terms (cases affected, not internal repair cost)',
      'Proposes a concrete backup/redundancy plan for sterile processing capacity',
      'Commits to a specific PM or monitoring change if this was preventable',
      'Does not overstate certainty about recurrence — realistic, not just reassuring',
      'Closes with a specific follow-up on the fix status',
    ],
  },
  {
    id: 'SL-FM-S1', day: 'Facilities', title: 'Preventive Maintenance Lapse Leads to a Resident Safety Incident',
    focus: 'Accountability with a family-facing client', difficulty: 'Advanced', icon: '🛗',
    industry: 'senior-living', trainingType: 'facilities-maintenance',
    context: "A grab bar in a resident's bathroom pulled loose from the wall, causing a fall. The resident is okay but shaken, and it turns out the quarterly bathroom safety-hardware inspection was overdue by five weeks. The Executive Director wants to understand what happened before she calls the family.",
    dataPacket: null,
    openingLine: "A resident fell because a grab bar came out of the wall. I need to know before I call his daughter — was this hardware supposed to be inspected, and was it?",
    clientPersona: `Executive Director of the community — protective of residents and acutely aware this is about to become a difficult family conversation for her. Not looking to assign blame yet, but needs the truth immediately.

What she actually cares about:
- The honest answer about whether the inspection was missed — she will find out either way
- What's being done right now to check every other unit for the same risk
- What she can accurately tell the family without over- or under-stating the facts

What she does NOT tolerate:
- Any hedging on whether the inspection was actually done — she needs the real answer now
- A GM trying to manage her messaging to the family rather than just giving her the facts

If the GM is anything less than fully direct about the missed inspection, she says: "I am about to call this man's daughter. I need the truth, not a version of it."`,
    successCriteria: [
      'States directly and immediately whether the inspection was missed — no hedging',
      'Proposes an immediate building-wide check of similar hardware, not just this unit',
      'Gives her accurate, usable information for the family call rather than trying to control her messaging',
      'Takes ownership of the process gap without becoming defensive',
      'Proposes a specific systemic fix (e.g., inspection tracking change) beyond just fixing this one bar',
      'Tone is grave and accountable, matching the seriousness of a resident injury',
    ],
  },
  {
    id: 'CD-FM-S1', day: 'Facilities', title: 'Facilities SLA Miss Raised at a QBR',
    focus: 'Data discipline in a formal review', difficulty: 'Foundational', icon: '📋',
    industry: 'corporate-dining', trainingType: 'facilities-maintenance',
    context: "At the quarterly business review, the property owner's facilities lead pulls up your work-order response-time data. Average response time has slipped from the 4-hour SLA to just under 7 hours over the quarter.",
    dataPacket: {
      title: 'Work Order Response Time — Trailing Quarter',
      headers: ['Month', 'SLA Target', 'Actual Avg Response', 'Ticket Volume'],
      rows: [
        ['Month 1', '4 hrs', '4.8 hrs', '112'],
        ['Month 2', '4 hrs', '6.5 hrs', '134'],
        ['Month 3', '4 hrs', '6.9 hrs', '141'],
      ],
    },
    openingLine: "Your contract says four hours. You're averaging almost seven. Walk me through why, and what changes next quarter.",
    clientPersona: `Facilities lead for the property owner — commercially minded, contract-literate, and reviewing this in front of other stakeholders in the room. Fair but expects a real explanation, not excuses.

What he actually cares about:
- Whether the volume increase (112→141) explains the slippage or whether it's a staffing/process issue
- A specific, credible plan to hit SLA next quarter — not just an apology
- Whether this trend threatens the broader relationship if unaddressed

What he does NOT ask about:
- Your internal technician wages or scheduling software — not his business
- Anything about your company's profitability

If the GM doesn't connect the ticket volume increase to the response time trend, he presses: "Is this a volume problem or a staffing problem? Those are two very different fixes."`,
    successCriteria: [
      'Connects the SLA miss to its actual driver (volume increase vs. staffing gap) using the data shown',
      'Does not hide behind the volume increase as a full excuse — takes ownership of the response gap',
      'Proposes a specific, measurable corrective plan for next quarter',
      'Avoids disclosing internal cost/staffing figures not relevant to the client conversation',
      'Sets a specific re-check point (e.g., mid-quarter review) rather than waiting for the next QBR',
      'Professional, non-defensive tone despite being called out in front of others',
    ],
  },
  {
    id: 'K12-FM-S1', day: 'Facilities', title: 'Emergency Repair During Instructional Time',
    focus: 'Stakeholder communication under time pressure', difficulty: 'Foundational', icon: '🏫',
    industry: 'k12', trainingType: 'facilities-maintenance',
    context: "A boiler failure has left an elementary school without heat on a 28-degree day. The Superintendent is fielding calls from worried parents and needs a plan he can communicate to the district within the hour.",
    dataPacket: null,
    openingLine: "I've got parents calling the district office asking if their kids are sitting in a cold building. I need a plan I can put out in the next thirty minutes.",
    clientPersona: `Superintendent — politically exposed (school board, parents, local news risk) and needs something concrete to communicate fast, not a technical explanation.

What he actually cares about:
- Whether students are safe and warm right now — portable heaters, relocated classrooms, or early dismissal
- A specific timeline he can put in a parent communication
- Confidence that this won't recur mid-winter

What he does NOT ask about:
- Boiler make/model or technical failure mode
- Contractor invoicing or repair cost details

If the GM's answer isn't something he can literally copy into a parent email, he says: "I need something I can send to three thousand parents in the next half hour — give me the actual plan."`,
    successCriteria: [
      'Leads with the immediate student-safety plan (heaters, relocation, or early dismissal) before anything else',
      'Gives language specific enough to use directly in a parent communication',
      'Provides a real repair timeline, not a vague one',
      'Addresses recurrence risk given the season without being asked',
      'Sets a specific next check-in time',
      'Calm, decisive tone appropriate to a superintendent under public pressure',
    ],
  },
// ═══════════════════════════ FACILITIES — HOUSEKEEPING (Client) ═══════════════════════════
  {
    id: 'HE-FH-S1', day: 'Facilities', title: 'Turnover-Week Staffing Shortfall Threatens the Cleaning Timeline',
    focus: 'Proactive communication under staffing strain', difficulty: 'Foundational', icon: '🧹',
    industry: 'higher-ed', trainingType: 'facilities-housekeeping',
    context: "Residence hall turnover week is in three days. Two housekeeping staff quit this week and a third is out on leave, leaving the team short-staffed for a hard deadline the Director of Housing Operations has already communicated to incoming students.",
    dataPacket: {
      title: 'Turnover Week Staffing Plan',
      headers: ['Role', 'Budgeted Staff', 'Currently Available', 'Gap'],
      rows: [['Housekeeping — Turnover Team', '14', '11', '-3'], ['Floor Techs', '3', '3', '0']],
    },
    openingLine: "You told me two weeks ago this timeline was solid. Now you're telling me you're short-staffed three days out. What changed, and are we going to make the move-in deadline?",
    clientPersona: `Director of Housing Operations — has already committed a room-ready date to families; a miss here becomes her problem publicly, not just yours.

What she actually cares about:
- A direct answer on whether the deadline will actually be hit
- A specific staffing/coverage plan (overtime, temp staff, reprioritized rooms) — not just an explanation of why they're short
- Being told this proactively rather than her finding out from a parent complaint

What she does NOT ask about:
- Why individual staff quit — not her concern unless it affects the timeline
- Your labor budget or overtime cost

If the GM is vague about whether the deadline will be met, she presses: "Yes or no — will every room be ready by move-in day? I need a real answer, not 'we're working on it.'"`,
    successCriteria: [
      'Gives a direct, honest answer on deadline feasibility rather than a vague reassurance',
      'Presents a specific coverage plan (overtime, temp staff, room prioritization)',
      'Owns that this should have been flagged proactively rather than reactively',
      'Does not over-promise a timeline that isn\'t realistic',
      'Proposes a specific check-in cadence over the next three days',
      'Stays calm and solution-focused under her direct pressure',
    ],
  },
  {
    id: 'SL-FH-S1', day: 'Facilities', title: 'Infection-Control Cleaning Lapse Flagged During a State Survey',
    focus: 'Regulatory accountability', difficulty: 'Advanced', icon: '🧴',
    industry: 'senior-living', trainingType: 'facilities-housekeeping',
    context: "A state surveyor doing an annual inspection found terminal cleaning logs for a resident's room incomplete after a C. diff isolation period ended. The Executive Director needs to understand the gap before responding to the surveyor.",
    dataPacket: null,
    openingLine: "The surveyor just told me our terminal cleaning documentation has a gap on an isolation room. That is a citation risk. What actually happened?",
    clientPersona: `Executive Director — this directly affects the facility's state survey outcome and, by extension, its reputation and census. Serious, precise, needs facts fast.

What she actually cares about:
- Whether the cleaning itself was actually done correctly, or just undocumented (very different problems)
- Immediate corrective action she can report to the surveyor
- Whether this is isolated or reflects a broader documentation gap

What she does NOT tolerate:
- Conflating "the cleaning happened but wasn't logged" with "we're not sure if it happened" — she needs to know which it is
- Vagueness in a moment where the surveyor is actively deciding on a citation

If the GM can't clearly distinguish execution failure from documentation failure, she says: "Those are two completely different problems. Which one is this?"`,
    successCriteria: [
      'Clearly distinguishes whether this is a documentation gap or an actual cleaning-protocol failure',
      'Provides immediate corrective action appropriate to whichever it is',
      'Gives her something specific and accurate to tell the surveyor',
      'Does not minimize the seriousness of an infection-control finding',
      'Proposes a systemic fix (e.g., real-time log verification) not just a one-off correction',
      'Direct, serious tone matching the survey stakes',
    ],
  },
  {
    id: 'AC-FH-S1', day: 'Facilities', title: 'HAI Cluster Traced to Terminal Cleaning Gaps',
    focus: 'High-stakes escalation to a clinical leader', difficulty: 'Advanced', icon: '🦠',
    industry: 'acute-care', trainingType: 'facilities-housekeeping',
    context: "Infection Prevention has identified a small cluster of hospital-acquired infections on one unit and traced part of the root-cause investigation to inconsistent terminal cleaning between patients. The Chief Nursing Officer wants answers directly from you before the outbreak investigation is closed.",
    dataPacket: {
      title: 'Terminal Clean Audit — Affected Unit, Past 30 Days',
      headers: ['Metric', 'Target', 'Actual', 'Gap'],
      rows: [['UV verification pass rate', '100%', '81%', '-19 pts'], ['Documented turnaround time', '<45 min', '62 min avg', '+17 min']],
    },
    openingLine: "Infection Prevention has traced part of this cluster back to cleaning gaps on your team. I need to understand exactly what broke down, because patients were harmed.",
    clientPersona: `Chief Nursing Officer — this is a patient-harm event, the most serious category of client conversation in this account. She is not hostile, but this is not a normal operational review.

What she actually cares about:
- A complete, honest account of what the audit data shows — no minimizing
- Immediate corrective action for the affected unit
- A credible plan that will hold up if this becomes a formal root-cause analysis with regulators

What she does NOT tolerate:
- Any defensiveness or minimizing given that patients were affected
- Vague commitments — she needs a specific, verifiable action plan

If the GM doesn't immediately acknowledge the seriousness, she says: "I need you to understand — this isn't a KPI miss. Patients got infections. Treat it like that."`,
    successCriteria: [
      'Immediately acknowledges the severity — a patient-harm event, not a routine metric miss',
      'Speaks honestly to what the audit data shows without minimizing',
      'Proposes immediate corrective action (verification step, retraining) for the affected unit',
      'Offers a credible plan that could hold up under a formal root-cause review',
      'Does not become defensive despite the gravity of the conversation',
      'Commits to specific follow-up reporting to Infection Prevention',
    ],
  },
  {
    id: 'AC-FH-S2', day: 'Facilities', title: 'Discharge-to-Clean Turnaround Time Missed, Backing Up the ED',
    focus: 'Operational escalation under capacity pressure', difficulty: 'Foundational', icon: '⏱️',
    industry: 'acute-care', trainingType: 'facilities-housekeeping',
    context: "Room turnaround time between discharge and ready-for-next-patient has slipped, and it's now contributing to ED boarding — patients waiting in the emergency department for an inpatient bed. The Director of Patient Flow wants this fixed today.",
    dataPacket: {
      title: 'Discharge-to-Clean Turnaround',
      headers: ['Metric', 'Target', 'This Week Avg'],
      rows: [['Turnaround time', '30 min', '52 min']],
    },
    openingLine: "I have patients boarding in the ED right now partly because rooms aren't turning around fast enough. This is a today problem, not a next-week problem.",
    clientPersona: `Director of Patient Flow — measured entirely on capacity and throughput; a slow room turnaround is a direct hit to her core metric and, by extension, patient care.

What she actually cares about:
- Whether this can genuinely be improved today, not just going forward
- The specific bottleneck in the turnaround process
- A number she can track this week to know if it's working

What she does NOT ask about:
- Staffing schedules or internal team management
- Cleaning supply costs

If the GM proposes something vague like "we'll prioritize it," she pushes: "Prioritize it how, specifically? What changes today?"`,
    successCriteria: [
      'Identifies the specific bottleneck rather than a generic "we\'ll work on it"',
      'Proposes a concrete change effective today, not next week',
      'Gives her a specific target and tracking method for this week',
      'Understands and speaks to the downstream ED impact, not just the cleaning metric in isolation',
      'Sets a same-day or next-day follow-up',
      'Urgent but composed tone matching the capacity pressure',
    ],
  },
  {
    id: 'CD-FH-S1', day: 'Facilities', title: 'Visible Cleanliness Complaint Ahead of a Board Visit',
    focus: 'Executive-level standards conversation', difficulty: 'Foundational', icon: '✨',
    industry: 'corporate-dining', trainingType: 'facilities-housekeeping',
    context: "The company's Board of Directors is touring the headquarters campus next week, including the executive dining area. The facilities director noticed visible dust and streaked glass in the executive dining room during a walkthrough today.",
    dataPacket: null,
    openingLine: "The Board is here next Tuesday, and today I walked through the executive dining room and saw dust on the sills and streaks on the glass. That can't happen next week.",
    clientPersona: `Facilities Director for the corporate client — image-conscious given the upcoming visit, but reasonable if given a real plan rather than defensiveness.

What he actually cares about:
- A specific plan to elevate standards before Tuesday, not just an apology for today
- Whether this is a one-off oversight or reflects a broader standards gap
- Confidence the space will hold up under scrutiny, not just look good for one day

What he does NOT ask about:
- Your janitorial staffing levels or shift schedules
- Cleaning product costs

If the GM's response feels like it's just about fixing today's issue rather than sustaining it through the visit, he asks: "What about the day after the Board leaves — does this hold, or does it slip right back?"`,
    successCriteria: [
      'Proposes a specific elevated-standard plan through the visit date, not just a one-time fix',
      'Addresses whether this is isolated or systemic honestly',
      'Proposes a sustainability plan beyond the visit itself, not just a one-day push',
      'Does not overreact defensively to a fair critique',
      'Offers a specific pre-visit walkthrough/inspection to confirm readiness',
      'Professional, confident tone appropriate to an executive-facing space',
    ],
  },
  {
    id: 'K12-FH-S1', day: 'Facilities', title: 'Restroom Cleanliness Complaint at a School Board Meeting',
    focus: 'Public-facing accountability', difficulty: 'Foundational', icon: '🏫',
    industry: 'k12', trainingType: 'facilities-housekeeping',
    context: "A parent raised a complaint about restroom cleanliness at last night's public school board meeting, and it made the district's social media. The Superintendent wants a direct answer and a plan he can reference publicly if asked again.",
    dataPacket: null,
    openingLine: "This complaint is now public — it was raised at the board meeting and it's on our district Facebook page. I need to know what's really going on and what I can say if a board member asks me directly.",
    clientPersona: `Superintendent — politically exposed, needs something specific and credible he can say publicly, not internal operational detail.

What he actually cares about:
- Whether the complaint reflects a real, ongoing issue or an isolated incident
- Language he can use if asked about this publicly again
- A visible, verifiable improvement he can point to

What he does NOT ask about:
- Cleaning schedules or staffing specifics beyond what's needed for his public answer
- Internal budget details

If the GM's answer wouldn't hold up if he had to repeat it publicly, he says: "I might get asked this exact question again in public. Give me something I can actually say."`,
    successCriteria: [
      'Gives an honest assessment of whether the complaint reflects an ongoing issue',
      'Provides specific, public-ready language rather than internal jargon',
      'Proposes a visible corrective step the Superintendent can reference if asked again',
      'Does not minimize a parent complaint that\'s already gone public',
      'Offers a specific timeframe for a visible improvement',
      'Composed tone appropriate to a politically sensitive, public situation',
    ],
  },
  {
    id: 'CD-FH-S2', day: 'Facilities', title: 'Post-Event Cleanup Failure After a High-Profile Client Event',
    focus: 'Service recovery after a visible failure', difficulty: 'Advanced', icon: '🎉',
    industry: 'corporate-dining', trainingType: 'facilities-housekeeping',
    context: "A major client-facing event was held in the atrium last night. This morning, the space wasn't fully reset before the first meetings of the day — chairs still stacked, spills not addressed. The HR partner who organized the event is furious and has already emailed several executives.",
    dataPacket: null,
    openingLine: "I already had three executives ask me this morning why the atrium looked like a frat house after last night's event. I need to know what happened and I need it fixed before this becomes a bigger problem.",
    clientPersona: `HR Partner who organized the event — personally embarrassed in front of leadership, frustrated, and wants both an immediate fix and real accountability.

What she actually cares about:
- The space being fully corrected immediately, not "in a bit"
- A clear reason this happened — short staffing, miscommunication, or something else
- Confidence this won't happen at the next executive event she plans

What she does NOT tolerate:
- Excuses that don't come with an immediate fix
- Vagueness about whether it will happen again

If the GM doesn't address the immediate state of the space first, she cuts in: "I don't need the explanation yet — is someone in there right now fixing it?"`,
    successCriteria: [
      'Leads with immediate action to fix the space right now, before explaining what happened',
      'Gives a clear, honest reason for the failure once the immediate fix is addressed',
      'Proposes a specific safeguard for future high-profile events (e.g., post-event checklist, sign-off)',
      'Acknowledges her embarrassment in front of leadership without being overly apologetic or groveling',
      'Offers concrete reassurance tied to an actual process change, not just a promise',
      'Recovers the relationship rather than just closing the immediate complaint',
    ],
  },
]
...
// ═══════════════════════════ FACILITIES — MAINTENANCE (Leadership) ═══════════════════════════
  {
    id: 'HE-FM-L1', day: 'Leadership', title: 'Technician Skipping PM Checklist Steps to Hit Close Rate',
    focus: 'Coaching without demotivating', difficulty: 'Foundational', icon: '📋',
    industry: 'higher-ed', trainingType: 'facilities-maintenance-leadership',
    context: "You review completed preventive-maintenance work orders and notice a technician is closing PM tickets in a fraction of the expected time, consistently skipping documented inspection steps. His close rate is the best on the team.",
    dataPacket: { title: 'PM Checklist Compliance — Past Month', headers: ['Technician', 'Avg PM Time', 'Team Avg', 'Steps Skipped (audit sample)'], rows: [['This Technician', '12 min', '28 min', '4 of 9 steps missing'], ['Team Average', '28 min', '—', '0-1 steps missing']] },
    openingLine: "I get my PMs done faster than anyone on this team. Why is that suddenly a problem?",
    counterpartPersona: `A technician who's fast, confident, and genuinely believes speed is what's valued — because it's what's been implicitly rewarded until now. Not lazy, just optimizing for the wrong signal.

What he actually cares about:
- Not being accused of being lazy when he thinks he's the hardest worker on the team
- A clear, specific explanation of which steps matter and why
- Not losing status as the "fast one" on the team

How he behaves:
- If just told to "slow down," he gets defensive: "So now going fast is a bad thing?"
- If shown specifically which steps are being skipped and why they matter (failure prevention, not box-checking), he engages more seriously
- Responds well to being asked to demonstrate the full checklist rather than just being told he's wrong`,
    successCriteria: [
      'Reframes the conversation around risk, not speed — explains why the specific skipped steps matter',
      'Avoids accusing him of laziness given his high output elsewhere',
      'Has him walk through the full checklist together rather than lecturing',
      'Distinguishes this from a discipline conversation — it\'s a training/standards reset',
      'Sets a specific follow-up audit to confirm the change sticks',
      'Leaves him feeling corrected on standards, not devalued as a performer',
    ],
  },
  {
    id: 'AC-FM-L1', day: 'Leadership', title: 'Union Grievance Over Overtime Distribution',
    focus: 'Fair process under scrutiny', difficulty: 'Advanced', icon: '⚖️',
    industry: 'acute-care', trainingType: 'facilities-maintenance-leadership',
    context: "A union technician filed a grievance claiming overtime has been distributed unfairly, favoring certain technicians during a short-staffed stretch. The union steward wants to discuss it with you before it escalates to a formal hearing.",
    dataPacket: { title: 'OT Hours Distribution — Past 6 Weeks', headers: ['Technician', 'OT Hours', 'Seniority Rank'], rows: [['Tech A', '38 hrs', '2nd'], ['Tech B (grievant)', '6 hrs', '1st'], ['Tech C', '31 hrs', '4th']] },
    openingLine: "Your seniority list says overtime goes by seniority first. Tech B has more seniority than half the people getting OT and barely any hours. Explain that to me.",
    counterpartPersona: `A union steward — procedural, not personally hostile, but will hold you to the exact language of the contract's OT distribution clause.

What he actually cares about:
- Whether the contract's overtime-by-seniority process was actually followed
- A fair, verifiable explanation if seniority was bypassed for a legitimate reason (e.g., specialized certification needed)
- Setting a clear precedent so this doesn't recur

How he behaves:
- If the GM is vague or seems to be improvising a justification, he escalates: "That's not what's written in the contract. I need the actual process you followed."
- If given a clear, contract-consistent explanation (even an admitted mistake), he's willing to resolve it informally`,
    successCriteria: [
      'Reviews the actual facts honestly before responding — doesn\'t guess or bluff',
      'If a genuine process error occurred, admits it directly rather than rationalizing after the fact',
      'If there was a legitimate skills-based exception, explains it clearly with reference to the contract language',
      'Proposes a concrete process fix to prevent recurrence',
      'Treats the steward as a legitimate process partner, not an adversary',
      'Resolves this informally where possible rather than letting it escalate unnecessarily',
    ],
  },
  {
    id: 'SL-FM-L1', day: 'Leadership', title: 'Vendor Missed SLA Repeatedly — Renegotiate or Replace',
    focus: 'Vendor accountability', difficulty: 'Advanced', icon: '🚚',
    industry: 'senior-living', trainingType: 'facilities-maintenance-leadership',
    context: "Your HVAC service contractor has missed their 24-hour emergency response SLA three times this quarter, most recently leaving a resident wing without functioning AC for two days in summer. You're meeting with their account manager to decide whether this relationship continues.",
    dataPacket: { title: 'HVAC Vendor SLA Performance — This Quarter', headers: ['Incident', 'SLA', 'Actual Response'], rows: [['Unit 4B chiller', '24 hrs', '31 hrs'], ['Unit 2 rooftop unit', '24 hrs', '46 hrs'], ['Resident wing outage', '24 hrs', '52 hrs']] },
    openingLine: "We value this relationship and we're bringing on more technicians next quarter — I think you'll see improvement.",
    counterpartPersona: `The vendor's account manager — wants to preserve the contract and is offering reassurance, but hasn't yet offered anything concrete or measurable.

What he actually cares about:
- Keeping the account without a hard commitment that exposes his company to penalty
- Avoiding a formal SLA-breach conversation if he can smooth it over with reassurance

How he behaves:
- If the GM accepts vague reassurance, he offers nothing more concrete
- If the GM insists on specific, measurable commitments (response time guarantee, penalty clause, backup vendor plan), he engages more seriously and offers real terms
- Will resist penalty clauses initially but concede them if the GM holds firm and cites the actual incident data`,
    successCriteria: [
      'Rejects vague reassurance and insists on specific, measurable commitments',
      'Uses the actual incident data to make the case, not general frustration',
      'Proposes a concrete consequence structure (e.g., SLA penalty, probation period) if performance doesn\'t improve',
      'Considers and raises a backup-vendor contingency given resident safety was affected',
      'Keeps the tone professional — this is a business decision, not a personal conflict',
      'Leaves the meeting with a specific, written commitment rather than a verbal assurance',
    ],
  },
  {
    id: 'CD-FM-L1', day: 'Leadership', title: 'Capital Repair vs. Run-to-Failure Budget Tradeoff',
    focus: 'Cost discipline conversation with your team', difficulty: 'Advanced', icon: '💰',
    industry: 'corporate-dining', trainingType: 'facilities-maintenance-leadership',
    context: "Your maintenance supervisor wants to replace an aging rooftop unit proactively before it fails. Budget is tight this quarter and you have to decide, and explain to him, whether to approve the capital request or run the unit until it fails.",
    dataPacket: { title: 'Rooftop Unit — Cost Comparison', headers: ['Option', 'Cost', 'Risk'], rows: [['Proactive replacement', '$18,500', 'None — planned downtime'], ['Run to failure', '$0 now / est. $24,000 later', 'Unplanned outage risk, possible emergency premium']] },
    openingLine: "This unit is on borrowed time. If we wait for it to fail, we're looking at an emergency replacement and probably a week without proper cooling in that section. I don't understand why this isn't an easy yes.",
    counterpartPersona: `Your maintenance supervisor — technically right about the risk, frustrated that budget constraints seem to be overriding sound engineering judgment.

What he actually cares about:
- Being taken seriously as the technical expert, not overruled without explanation
- Understanding the actual budget constraint, not just being told "no"
- Confidence that if it does fail, there's a real contingency plan

How he behaves:
- If just told "no budget," he pushes back hard: "So we're just waiting for it to break?"
- If given the real budget picture and included in exploring alternatives (partial funding, phased approach, contingency plan), he engages constructively`,
    successCriteria: [
      'Explains the actual budget constraint honestly rather than just saying no',
      'Takes his technical risk assessment seriously rather than dismissing it',
      'Explores a middle path (partial funding, phased timeline, contingency plan) rather than a binary yes/no',
      'If the decision is genuinely to run-to-failure, builds a real contingency plan with him',
      'Doesn\'t make him feel like his expertise was ignored',
      'Documents the decision and rationale for accountability',
    ],
  },
  {
    id: 'K12-FM-L1', day: 'Leadership', title: 'Seasonal Staffing Crunch — Resistant Crew Lead',
    focus: 'Getting buy-in for a demanding schedule', difficulty: 'Foundational', icon: '📢',
    industry: 'k12', trainingType: 'facilities-maintenance-leadership',
    context: "Summer shutdown is the district's only window for major projects — floor stripping, HVAC overhauls — across a dozen buildings in eight weeks. Your crew lead thinks the schedule you've built is unrealistic and is pushing back hard in front of the team.",
    dataPacket: null,
    openingLine: "You want us to hit twelve buildings in eight weeks? That's not a schedule, that's a wish list.",
    counterpartPersona: `A veteran crew lead — has done summer shutdown for years, isn't wrong that it's demanding, and is testing whether you'll listen or just dictate.

What he actually cares about:
- Being consulted on a schedule that affects his crew, not handed one
- Whether the schedule accounts for real-world snags (material delivery, building access issues)
- Not looking like he's just complaining in front of the team

How he behaves:
- If the GM just insists the schedule stands without engaging his concerns, he disengages and the team follows his lead
- If the GM genuinely works through the schedule with him and adjusts based on legitimate concerns, he becomes an ally in selling it to the crew`,
    successCriteria: [
      'Takes his pushback seriously rather than just asserting authority',
      'Works through the schedule together, identifying which concerns are legitimate vs. which are just resistance',
      'Makes real adjustments where his concerns are valid',
      'Reframes the conversation as a private planning discussion rather than a public standoff',
      'Secures his buy-in as someone who will help sell the plan to the crew, not just comply',
      'Sets clear checkpoints to catch schedule slippage early',
    ],
  },
  {
    id: 'HE-FM-L2', day: 'Leadership', title: 'Safety Shortcut — Lockout/Tagout Skipped',
    focus: 'Accountability on a safety violation', difficulty: 'Advanced', icon: '🔒',
    industry: 'higher-ed', trainingType: 'facilities-maintenance-leadership',
    context: "You witnessed a technician working on an electrical panel without following lockout/tagout procedure. This is a serious safety violation, not a performance issue — it needs to be addressed immediately and directly.",
    dataPacket: null,
    openingLine: "It was a two-minute job. I've done it a hundred times without locking out. Nothing's ever happened.",
    counterpartPersona: `An experienced technician who has cut this corner for years without incident and genuinely doesn't see the immediate danger — normalized risk from repetition.

What he actually cares about:
- Not being treated like a rookie who doesn't understand electrical work
- Feeling like the rule is really about safety, not just liability-covering paperwork

How he behaves:
- If lectured generically about "following the rules," he tunes out: "I know the rules, I've just never needed them for something this quick"
- If confronted directly about the actual severity (this is a can-kill-you violation, not a minor infraction) and treated seriously rather than as a lecture, he's more likely to actually reconsider`,
    successCriteria: [
      'Treats this with the seriousness of a life-safety violation, not a routine coaching moment',
      'Directly names that "nothing\'s happened yet" is not evidence of safety',
      'Does not let experience or seniority soften the correction',
      'Reinforces LOTO as non-negotiable, with no minor-job exception',
      'Documents the conversation appropriately given the severity',
      'Closes with a clear, unambiguous expectation — not a soft "just be careful"',
    ],
  },
  {
    id: 'AC-FM-L2', day: 'Leadership', title: 'Cross-Training Pushback From a Senior Technician',
    focus: 'Change management with a resistant veteran', difficulty: 'Foundational', icon: '🔄',
    industry: 'acute-care', trainingType: 'facilities-maintenance-leadership',
    context: "You're rolling out cross-training so every technician can cover both HVAC and general maintenance calls, reducing coverage gaps. Your most senior technician, who has specialized in HVAC for 15 years, doesn't want to learn general maintenance work.",
    dataPacket: null,
    openingLine: "I've specialized in HVAC for fifteen years. Why am I suddenly being asked to learn plumbing repairs at this point in my career?",
    counterpartPersona: `A senior, highly skilled technician who takes pride in his specialization and sees this as a demotion of his expertise, not a skill addition.

What he actually cares about:
- Not having his specialized expertise devalued
- Understanding the actual reason for the change (coverage gaps), not just being told it's "policy"
- Some acknowledgment that this is a real ask of someone with his experience level

How he behaves:
- If it's framed as "everyone has to do this now," he feels flattened into a generic role and resists
- If the GM explains the specific coverage problem this solves and acknowledges his specialized value is still primary, he's more willing to engage with cross-training as an addition, not a replacement`,
    successCriteria: [
      'Explains the specific operational problem (coverage gaps) driving the change',
      'Acknowledges his specialized expertise remains valued, not diminished',
      'Frames cross-training as additive, not a replacement of his specialization',
      'Listens to and addresses his specific concerns rather than citing policy',
      'Finds a reasonable pace/scope for his cross-training given his experience level',
      'Secures genuine buy-in rather than reluctant compliance',
    ],
  },
  {
    id: 'SL-FM-L2', day: 'Leadership', title: 'Budget Cut Conversation — Reduced Parts and Supplies Budget',
    focus: 'Communicating constraints without demoralizing the team', difficulty: 'Advanced', icon: '📉',
    industry: 'senior-living', trainingType: 'facilities-maintenance-leadership',
    context: "Corporate has cut your maintenance parts and supplies budget by 15% for the coming year. You need to tell your team, who already feel stretched, without losing their morale or their willingness to flag issues honestly.",
    dataPacket: { title: 'Parts & Supplies Budget', headers: ['Category', 'Prior Year', 'New Budget', 'Change'], rows: [['Parts & Supplies', '$42,000', '$35,700', '-15%']] },
    openingLine: "We're already stretched thin as it is. Now you're telling us there's even less to work with?",
    counterpartPersona: `Represents the team's collective frustration through your most vocal technician — not unreasonable, but worried this means more corner-cutting will be expected of them.

What the team actually cares about:
- Whether this means they'll be pressured to skip needed repairs or use lower-quality parts
- Having a say in where the cuts actually land, rather than it being dictated
- Reassurance that safety-critical items aren't part of the cut

How they behave:
- If the cut is presented as just "do more with less," morale drops and corner-cutting risk increases
- If the GM is specific about where cuts will and won't apply (e.g., not safety-critical items) and involves the team in prioritization, they respond better even though the news is still bad`,
    successCriteria: [
      'Is direct and honest about the budget cut rather than softening it into vague language',
      'Specifically reassures that safety-critical repairs are not part of the reduction',
      'Involves the team in identifying where non-critical savings can come from',
      'Does not imply that quality or safety should be sacrificed to hit the number',
      'Acknowledges the team\'s existing workload pressure honestly',
      'Leaves the team informed and included, not just told what to do',
    ],
  },
// ═══════════════════════════ FACILITIES — HOUSEKEEPING (Leadership) ═══════════════════════════
  {
    id: 'HE-FH-L1', day: 'Leadership', title: 'Quality Score Decline Conversation With a Shift Supervisor',
    focus: 'Coaching a supervisor on team performance', difficulty: 'Foundational', icon: '📉',
    industry: 'higher-ed', trainingType: 'facilities-housekeeping-leadership',
    context: "Room inspection quality scores for the evening shift have declined two months running. The shift supervisor runs a good team but seems unaware of, or unwilling to address, the trend.",
    dataPacket: { title: 'Room Inspection Scores — Evening Shift', headers: ['Month', 'Avg Score', 'Target'], rows: [['Month 1', '91%', '90%'], ['Month 2', '84%', '90%'], ['Month 3', '79%', '90%']] },
    openingLine: "I didn't realize it had dropped that much. Honestly, the team seems fine to me.",
    counterpartPersona: `A shift supervisor who's well-liked by her team and conflict-averse — she may be aware of small slippages but hasn't wanted to confront individual performers about it.

What she actually cares about:
- Not feeling accused of not doing her job
- Having a specific, manageable way to address this rather than a vague "fix it"
- Not damaging her relationships with her team by suddenly cracking down

How she behaves:
- If just told to "raise your standards," she nods but nothing changes — too vague to act on
- If helped to identify the specific pattern (which rooms, which shift days) and coached on how to have the individual conversations she's avoiding, she engages more seriously`,
    successCriteria: [
      'Shows her the specific data pattern rather than a general "scores are down"',
      'Explores with her why she may have been avoiding individual accountability conversations',
      'Coaches her on how to have those specific conversations, not just tells her to have them',
      'Sets a specific, trackable target for next month',
      'Doesn\'t undermine her authority with her team in the process',
      'Follows up at a defined interval to check progress',
    ],
  },
  {
    id: 'SL-FH-L1', day: 'Leadership', title: 'Chemical Safety Protocol Violation',
    focus: 'Safety accountability', difficulty: 'Advanced', icon: '⚠️',
    industry: 'senior-living', trainingType: 'facilities-housekeeping-leadership',
    context: "You found a housekeeping aide mixing cleaning chemicals improperly and not wearing required PPE, creating a real safety hazard in a building full of vulnerable residents.",
    dataPacket: null,
    openingLine: "I've been doing it this way for two years. Nobody ever told me it was wrong.",
    counterpartPersona: `A housekeeping aide who genuinely may not have been trained properly — this could be a training gap, not willful negligence, though the risk is serious either way.

What she actually cares about:
- Not being blamed for something she may not have been properly trained on
- Understanding the actual danger, not just being told "that's the rule"
- Not feeling humiliated in front of residents or coworkers

How she behaves:
- If treated as if she willfully broke a known rule, she gets defensive and shuts down
- If the GM investigates whether this is a training gap first, and explains the real danger (chemical burns, toxic fumes) rather than just citing policy, she's more receptive and asks genuine questions`,
    successCriteria: [
      'Investigates whether this reflects a training gap before assuming willful violation',
      'Explains the actual danger specifically (chemical interaction risk) rather than just citing the rule',
      'Addresses this privately, protecting her dignity in front of residents/coworkers',
      'Provides or arranges proper retraining immediately given the safety stakes',
      'Verifies whether other staff have the same gap, not just this one person',
      'Sets a clear, documented expectation given the seriousness of the hazard',
    ],
  },
  {
    id: 'AC-FH-L1', day: 'Leadership', title: 'Isolation Room Cleaning Protocol Shortcut',
    focus: 'Accountability on an infection-control violation', difficulty: 'Advanced', icon: '🧫',
    industry: 'acute-care', trainingType: 'facilities-housekeeping-leadership',
    context: "You observed an environmental services tech skip a required disinfectant dwell time while cleaning an isolation room in order to keep pace with a heavy workload. This directly undermines the point of the isolation protocol.",
    dataPacket: null,
    openingLine: "I had six rooms to turn and one nurse manager standing over my shoulder asking why it wasn't done yet. Something had to give.",
    counterpartPersona: `A tech who is genuinely overwhelmed by competing pressure — speed demands from nursing versus protocol requirements from infection control — and made a bad call under real pressure.

What he actually cares about:
- Being heard about the real, conflicting pressure he's under, not just corrected
- Not being the only one held accountable when the workload itself is the root problem
- Understanding this isn't optional even under pressure

How he behaves:
- If just told "never skip dwell time" without acknowledging the workload pressure, he feels unheard and the underlying problem persists
- If the GM addresses both the non-negotiable nature of the protocol AND the real workload issue, he's more likely to genuinely commit rather than just say what's expected`,
    successCriteria: [
      'Makes clear the dwell time requirement is non-negotiable regardless of workload pressure',
      'Genuinely acknowledges the workload/pressure problem rather than dismissing it',
      'Commits to addressing the root workload issue (staffing, scheduling, or pushback to nursing on timing expectations)',
      'Does not let empathy for his situation soften the safety standard',
      'Documents appropriately given the infection-control stakes',
      'Leaves him with both accountability and a genuine plan to reduce the pressure that caused this',
    ],
  },
  {
    id: 'K12-FH-L1', day: 'Leadership', title: 'No-Show Pattern From Part-Time Staff During the School Year',
    focus: 'Accountability conversation with a pattern of absences', difficulty: 'Foundational', icon: '📅',
    industry: 'k12', trainingType: 'facilities-housekeeping-leadership',
    context: "A part-time evening custodian has no-showed four times in the past six weeks, each time leaving a school building short-staffed for closing procedures. You need to address the pattern directly.",
    dataPacket: { title: 'Attendance — Past 6 Weeks', headers: ['Scheduled Shifts', 'No-Shows', 'Late Notice'], rows: [['24', '4', '1 of 4']] },
    openingLine: "Stuff comes up sometimes. I don't think it's been that many times.",
    counterpartPersona: `A part-time employee juggling this job with school or another job — may be minimizing the pattern to himself, not necessarily being dishonest, but not taking it seriously enough.

What he actually cares about:
- Not being fired outright without a chance to explain or improve
- Understanding exactly what's expected going forward
- Not feeling like a "part-time" role means the standards don't matter

How he behaves:
- If confronted vaguely ("you need to do better"), he agrees but the pattern likely continues
- If shown the actual specific dates and impact, and given clear, concrete expectations with real consequences, he's more likely to either commit seriously or self-select out`,
    successCriteria: [
      'Presents the specific dates and pattern rather than a vague generalization',
      'Explains the concrete operational impact of each no-show (building left short at closing)',
      'Sets clear, specific expectations and consequences going forward',
      'Gives him a genuine chance to explain before assuming the worst',
      'Does not treat "part-time" as an excuse for lower accountability',
      'Documents the conversation given the pattern and potential next steps',
    ],
  },
  {
    id: 'CD-FH-L1', day: 'Leadership', title: 'Labor Cost Overrun From Event Turnover Overtime',
    focus: 'Cost accountability with your team', difficulty: 'Advanced', icon: '💵',
    industry: 'corporate-dining', trainingType: 'facilities-housekeeping-leadership',
    context: "Overtime for housekeeping has spiked due to back-to-back corporate events requiring fast turnovers. Your supervisor has been approving the overtime to keep up, but the budget impact is now significant and needs to change.",
    dataPacket: { title: 'Housekeeping OT — Past Month', headers: ['Week', 'Budgeted OT', 'Actual OT', 'Driver'], rows: [['Week 1', '6 hrs', '18 hrs', '3 back-to-back events'], ['Week 2', '6 hrs', '22 hrs', '4 back-to-back events'], ['Week 3', '6 hrs', '15 hrs', '2 back-to-back events']] },
    openingLine: "I was just trying to make sure every event space was ready on time. What was I supposed to do, leave it messy?",
    counterpartPersona: `A supervisor who made a reasonable short-term call under real event-scheduling pressure, but hasn't stepped back to see the budget pattern or push back on the underlying scheduling problem.

What she actually cares about:
- Not being blamed for a scheduling problem that isn't really within her control
- Having a real alternative to overtime, not just being told to stop using it
- Feeling like her judgment call under pressure is understood, not just criticized

How she behaves:
- If just told to cut overtime without addressing the event-scheduling root cause, she feels set up to fail again
- If the GM works with her on both the immediate cost issue and a structural fix (event scheduling gaps, staffing plan for high-volume weeks), she engages constructively`,
    successCriteria: [
      'Acknowledges her reasonable intent under pressure before addressing the cost problem',
      'Identifies the root cause (event scheduling density) rather than treating this as just an overtime-approval issue',
      'Proposes a structural fix — advance staffing plan for high-volume weeks, or pushback on event scheduling',
      'Sets a specific, realistic OT target with a real plan to hit it',
      'Does not make her feel blamed for a systemic scheduling gap',
      'Follows up at a specific point to confirm the fix is working',
    ],
  },
  {
    id: 'HE-FH-L2', day: 'Leadership', title: 'Coaching a Supervisor Avoiding a Hard Conversation',
    focus: 'Developing a supervisor\'s accountability skills', difficulty: 'Advanced', icon: '🗣️',
    industry: 'higher-ed', trainingType: 'facilities-housekeeping-leadership',
    context: "One of your housekeeping aides has a consistent quality problem, and your shift supervisor knows about it but has avoided addressing it directly for weeks, hoping it resolves itself.",
    dataPacket: null,
    openingLine: "I've mentioned it to her a couple times in passing. I guess it just hasn't sunk in yet.",
    counterpartPersona: `A supervisor who is conflict-averse and has a friendly relationship with the underperforming aide, making direct confrontation uncomfortable for her.

What she actually cares about:
- Not damaging her relationship with the aide, who she likes personally
- Not feeling like she's bad at her job because she's avoided this
- Having a concrete script or approach rather than just being told to "have the conversation"

How she behaves:
- If just told "you need to address this," she agrees but the pattern of avoidance likely continues
- If the GM helps her actually rehearse or plan the specific conversation, and normalizes that this is a hard but necessary supervisor skill, she's more likely to actually follow through`,
    successCriteria: [
      'Identifies that "mentioning it in passing" isn\'t the same as an actual accountability conversation',
      'Explores why she\'s been avoiding it (relationship discomfort) without judgment',
      'Helps her plan or rehearse the specific conversation rather than just directing her to have it',
      'Normalizes this as a core supervisor skill she can build, not a personal failing',
      'Sets a specific deadline for her to have the conversation',
      'Follows up to confirm it happened and coach on how it went',
    ],
  },
  {
    id: 'AC-FH-L2', day: 'Leadership', title: 'Cross-Department Friction — Housekeeping vs. Nursing Over Room Turnover Priority',
    focus: 'Cross-functional conflict resolution', difficulty: 'Advanced', icon: '🤝',
    industry: 'acute-care', trainingType: 'facilities-housekeeping-leadership',
    context: "Your housekeeping team and a nursing unit are in ongoing friction over room-turnover priority — nursing feels housekeeping is too slow, housekeeping feels nursing gives contradictory instructions about which rooms are urgent. You're meeting with the nurse manager to resolve it.",
    dataPacket: null,
    openingLine: "Your team keeps telling my nurses different things than what they tell me about which rooms are next. It's chaos.",
    counterpartPersona: `A nurse manager who is frustrated by the operational friction but ultimately wants the same outcome — patients placed quickly. Direct, but not unreasonable if given a real process fix.

What she actually cares about:
- A single, reliable point of coordination instead of conflicting information
- Rooms turning around fast enough to support patient flow
- Not having to personally chase down housekeeping status

How she behaves:
- If the GM is defensive about their team's role in the confusion, she escalates further
- If the GM proposes a clear, mutually agreed process (single point of contact, shared priority list), she engages constructively and is willing to hold her own team to it too`,
    successCriteria: [
      'Acknowledges housekeeping\'s part in the communication breakdown rather than only blaming nursing',
      'Proposes a specific, concrete coordination process (single point of contact, shared status board, etc.)',
      'Gets her agreement on nursing\'s side of the process too — this is a two-way fix',
      'Does not let this become a blame exchange rather than a solution conversation',
      'Ties the fix back to the shared goal — patient flow — rather than departmental turf',
      'Sets a specific follow-up to confirm the new process is working for both sides',
    ],
  },
  {
    id: 'SL-FH-L2', day: 'Leadership', title: 'Vendor Negotiation — Linen and Laundry Service Contract Renewal',
    focus: 'Cost negotiation through relationship', difficulty: 'Advanced', icon: '🧺',
    industry: 'senior-living', trainingType: 'facilities-housekeeping-leadership',
    context: "Your linen and laundry service contract is up for renewal, and the vendor is proposing a 12% increase citing rising labor and delivery costs. You need to negotiate this down while maintaining a service relationship you generally value.",
    dataPacket: { title: 'Proposed Renewal Terms', headers: ['Item', 'Current Rate', 'Proposed Rate', 'Change'], rows: [['Per-pound processing', '$0.42', '$0.47', '+12%'], ['Delivery frequency', '5x/week', '5x/week', 'No change'], ['Contract term', '1 year', '2 year (proposed)', 'Longer lock-in']] },
    openingLine: "Costs have gone up across the board for us too — labor, fuel, everything. This is really just us passing that through.",
    counterpartPersona: `A vendor account manager who has a genuine cost story but is also using it as an opportunity to lock in a longer, higher-margin contract term.

What he actually cares about:
- Retaining the account and, ideally, locking in a longer contract term
- Not looking like he gave away margin without getting something in return

How he behaves:
- If the GM accepts the increase without pushing on the contract term or any other trade-off, he happily takes it as-is
- If the GM negotiates specifically — smaller increase, shorter term, or volume/service trade-offs — he engages seriously since retaining the account still matters to him`,
    successCriteria: [
      'Doesn\'t accept the full proposed increase without negotiating',
      'Recognizes and addresses the longer contract term as a separate negotiating point from the price increase',
      'Proposes a specific counter (smaller increase, current contract length, or a service-level trade-off)',
      'Maintains the relationship — professional, not adversarial',
      'Reaches a concrete resolution rather than leaving it open-ended',
      'Documents the final agreed terms clearly',
    ],
  },
]
...
// ─── Service Line Overrides ─────────────────────────────────────────────────
// Stakeholder Mapping, Financial Storytelling, and QBR Builder were originally
// authored assuming a dining contract. Facilities — Maintenance and
// Facilities — Housekeeping are different service lines with different
// stakeholders, different metrics, and different CFO/exec pushback — so each
// gets its own override here. Overrides are written as functions of the base
// industry config so copy can reference siteTerm/decisionMakerTitle and stay
// consistent with the industry (e.g. "campus" vs "community" vs "facility").
// A missing/unknown serviceLine (dining tracks) falls back to the industry's
// own dining-flavored config untouched — see getIndustryConfig below.

export const SERVICE_LINE_CONFIG = {
  'facilities-maintenance': (base) => ({
    stakeholderRoles: [
      { key: 'exec_sponsor',  role: 'Executive Sponsor (President/CEO-level)', cares: `Reputation and safety risk, no surprises reaching the board about the physical ${base.siteTerm}` },
      { key: 'cfo',           role: `${base.decisionMakerTitle} / Finance`,     cares: 'Capital spend, contract performance, budget variance — signs the contract' },
      { key: 'ops_leader',    role: 'VP Operations / COO',                      cares: `Uptime and service reliability across the ${base.siteTerm}, cross-department coordination during outages` },
      { key: 'facilities_liaison', role: 'Director of Facilities (client-side)', cares: 'Work order performance, PM compliance, code/life-safety compliance — your direct day-to-day counterpart' },
      { key: 'safety',        role: 'EHS / Risk & Safety Manager',              cares: 'Recordable incidents, code violations, insurance and liability exposure' },
      { key: 'occupant_rep',  role: 'Occupant/Department Leadership',           cares: `Advance notice before outages or shutdowns, timely resolution, disruption to the people who use the ${base.siteTerm} day to day` },
      { key: 'events',        role: 'Director of Events / Scheduling',          cares: 'Building and space availability, outage timing around scheduled events' },
      { key: 'labor_rep',     role: 'Union Steward / Labor Representative',     cares: 'Fair scheduling, overtime distribution, working conditions — where applicable' },
    ],
    financialMetrics: [
      { id: 'workOrderResponse', label: 'Work Order Response Time',    placeholder: 'e.g. 6.8 hrs vs 4 hr SLA', internal: 'Average time to first response on work orders vs. the contracted SLA',
        defaultValue: '6.8 hrs (SLA: 4 hrs)', defaultTranslation: '' },
      { id: 'pmCompliance',      label: 'PM Compliance %',             placeholder: 'e.g. 91%',       internal: '% of scheduled preventive maintenance completed on time',
        defaultValue: '91%', defaultTranslation: 'Preventive maintenance compliance is running at 91% this quarter, meaning 9% of scheduled PM tasks slipped past their window. We have added a technician shift to close that gap by next quarter.' },
      { id: 'backlogAging',      label: 'Work Order Backlog (45+ days)', placeholder: 'e.g. 14 open',  internal: 'Number of work orders open longer than 45 days',
        defaultValue: '14 open 45+ days', defaultTranslation: 'We have 14 work orders open more than 45 days, down from 22 last quarter. Eight are pending backordered parts — outside our direct control — and the remaining six are scheduled for completion by month end.' },
      { id: 'laborPct',          label: 'Labor / Overtime %',          placeholder: 'e.g. 34.1%',     internal: 'Technician labor, including overtime, as a % of contract labor budget',
        defaultValue: '34.1% (OT: 6.2%)', defaultTranslation: 'Labor is running at 34.1% of budget, including 6.2 points of overtime driven by a short-staffed stretch covering two open technician positions. We have two offers out and expect to be fully staffed by next month.' },
      { id: 'capitalBacklog',    label: 'Deferred Capital Backlog',    placeholder: 'e.g. $340K',     internal: 'Dollar value of identified capital repair/replacement needs not yet funded',
        defaultValue: '$340K identified, unfunded', defaultTranslation: 'We have identified $340K in deferred capital needs, primarily roofing and HVAC equipment nearing end of life. This is a capital planning decision for you, not an operational shortfall on our part — we would like 30 minutes on your calendar to walk through options.' },
      { id: 'safetyIncidents',   label: 'Safety Incidents',            placeholder: 'e.g. 2 recordable', internal: 'Recordable safety incidents this period',
        defaultValue: '2 recordable, 0 lost-time', defaultTranslation: '' },
      { id: 'partsImpact',       label: 'Parts / Materials Cost Impact', placeholder: 'e.g. +9.2% HVAC parts', internal: 'Market-driven parts and materials cost increases outside your control',
        defaultValue: '+9.2% HVAC parts (national index)', defaultTranslation: 'Parts and materials costs are up 9.2% nationally per industry index data — a market condition affecting every provider, not something specific to how we are running this account.' },
    ],
    financialChallenges: [
      "Your work order backlog keeps growing and now you want another technician? Convince me that's not just a staffing excuse.",
      "PM compliance is under 95% again. What exactly am I paying for if preventive maintenance isn't getting done on schedule?",
      `I'm looking at $340K in deferred capital needs I didn't know about until this report. Why am I hearing about this now, and what happens if I don't fund it?`,
      "Response times slipped from a 4-hour SLA to almost 7. That's not a small miss. What's actually going on?",
    ],
    qbrSections: [
      { id: 'executiveSummary', label: 'Executive Summary', desc: 'Lead with the headline — what is the one thing leadership should take away from this QBR?',
        defaultValue: `Q2 held steady on core service levels with two items needing your attention. PM compliance improved to 91% and the aged work order backlog is down from 22 to 14. Response times slipped to 6.8 hours against our 4-hour SLA due to two open technician positions, both now in the offer stage. We have also identified $340K in deferred capital needs, primarily aging HVAC and roofing, that requires your direction before we can plan Fall work.` },
      { id: 'financialPerformance', label: 'Financial & Service Performance', desc: 'Response time, PM compliance, labor, and capital — in client language, not operator language.',
        defaultValue: `Work order response time averaged 6.8 hours against our 4-hour SLA, driven by two open technician positions covered through overtime, which pushed labor to 34.1% of budget with 6.2 points of that in OT. PM compliance held at 91%. The aged backlog (45+ days) is down to 14 open orders, eight of which are pending backordered parts outside our control. We have identified $340K in deferred capital needs — primarily HVAC and roofing nearing end of life — that is a funding decision for you, not an operational gap on our part.` },
      { id: 'experienceSection', label: 'Occupant Impact & Safety', desc: 'What building occupants experienced, what it means, and what you are doing about it.',
        defaultValue: `We logged 2 recordable safety incidents this quarter, 0 lost-time, both minor and closed out with corrective action. The most visible occupant impact was the extended response window on the two open technician roles; we proactively notified affected department leads ahead of any planned outage this quarter and had zero unplanned disruptions to scheduled events or classes.` },
      { id: 'strategicInitiatives', label: 'Strategic Initiatives', desc: 'What are you building or improving that creates value beyond the contract scope?',
        defaultValue: '' },
      { id: 'lookingAhead', label: 'Looking Ahead', desc: 'What decisions do you need from leadership in the next 30-60 days?',
        defaultValue: `Two items need your direction before Fall. First, the $340K deferred capital backlog on HVAC and roofing — we have priced a phased three-year plan and need your sign-off to include it in next year's capital budget. Second, we would like your alignment on a revised PM staffing model to keep compliance above 95% as the ${base.siteTerm} grows.` },
    ],
    qbrPersonas: [
      { id: 'cfo', label: `${base.decisionMakerTitle} / Finance`, icon: '💼', style: `Focused on what the ${base.siteTerm} pays and receives in return. Will probe capital requests, billing increases, and overtime spend. Will NOT ask about the provider's internal labor margins — that is confidential. Expects the GM to speak in client dollars and risk, not technician-level detail.` },
      { id: 'ops_leader', label: 'VP Operations / COO', icon: '🏗️', style: `Cares about uptime, reliability, and whether outages are communicated ahead of time. Will push hard on any missed SLA and ask for a specific corrective plan with a date attached.` },
      { id: 'exec_sponsor', label: 'Executive Sponsor', icon: '🏛️', style: `Focused on reputation and risk — wants the headline story on safety and whether the provider is ahead of problems or behind them. Expects the GM to speak like a strategic partner, not a vendor reading a checklist.` },
    ],
    diagnosticQuestions: [
      { id: 'financial',    label: `Explaining a missed SLA or capital request to a ${base.decisionMakerTitle} in client language` },
      { id: 'challenge',    label: `Responding to a ${base.decisionMakerTitle} who questions a staffing or scope expansion request` },
      { id: 'contract',     label: 'Reading and understanding your own contract, including SLA definitions' },
      { id: 'stakeholder',  label: `Identifying who informally influences facilities decisions at your ${base.siteTerm}` },
      { id: 'executive',    label: 'Delivering a structured update to an executive under pressure' },
      { id: 'recovery',     label: 'Having a direct conversation about a service failure, outage, or safety incident' },
      { id: 'qbr',          label: 'Presenting a QBR to a mixed executive audience' },
      { id: 'scope',        label: 'Saying "let me price that" when a client asks for more service' },
      { id: 'retention',    label: 'Seeing client retention and safety risk signals early enough to respond' },
      { id: 'relationships',label: 'Building genuine relationships above your primary contact' },
    ],
  }),
  'facilities-housekeeping': (base) => ({
    stakeholderRoles: [
      { key: 'exec_sponsor',  role: 'Executive Sponsor (President/CEO-level)', cares: `Reputation, appearance, and inspection outcomes at the ${base.siteTerm}` },
      { key: 'cfo',           role: `${base.decisionMakerTitle} / Finance`,     cares: 'Labor cost, contract performance, cost transparency — signs the contract' },
      { key: 'facilities_liaison', role: 'Director of Facilities / Environmental Services', cares: 'Inspection scores, turnover time, complaint trends — your direct day-to-day counterpart' },
      { key: 'infection_control', role: 'Infection Control / Risk Manager',     cares: 'Cleaning protocol compliance, audit and survey readiness, outbreak response' },
      { key: 'occupant_rep',  role: 'Occupant/Department Leadership',           cares: `Turnaround time on their spaces, responsiveness to complaints, cleanliness as it reflects on them` },
      { key: 'events',        role: 'Director of Events / Scheduling',          cares: 'Space readiness and turn time ahead of high-visibility events, tours, or move-ins' },
      { key: 'labor_rep',     role: 'Union Steward / Labor Representative',     cares: 'Fair scheduling, staffing levels, working conditions — where applicable' },
    ],
    financialMetrics: [
      { id: 'inspectionScore',   label: 'Inspection / Audit Score',    placeholder: 'e.g. 88%',       internal: '% score on the most recent formal cleanliness/compliance audit',
        defaultValue: '88%', defaultTranslation: '' },
      { id: 'inspectionChg',     label: 'Inspection Score Change',     placeholder: 'e.g. -5 pts',    internal: 'Change in inspection score vs. prior period in percentage points',
        defaultValue: '-5 pts', defaultTranslation: 'Our inspection score dropped 5 percentage points this period, from 93% to 88%. Three of the five points came from a single unit during a period we were covering two open positions; the other two reflect a documentation gap we have already corrected.' },
      { id: 'turnoverTime',      label: 'Space Turnover Time',         placeholder: 'e.g. 42 min vs 30 min target', internal: 'Average time to fully turn a space (room, unit, or area) vs. target',
        defaultValue: '42 min (target: 30 min)', defaultTranslation: 'Average turnover time is running 42 minutes against our 30-minute target, concentrated during the highest-volume shift. We have adjusted staggered staffing starting next week to close that gap.' },
      { id: 'laborPct',          label: 'Labor / Overtime %',          placeholder: 'e.g. 36.4%',     internal: 'Housekeeping labor, including overtime, as a % of contract labor budget',
        defaultValue: '36.4% (OT: 5.1%)', defaultTranslation: 'Labor is running at 36.4% of budget, including 5.1 points of overtime tied to covering two open positions. We have one hire starting in two weeks.' },
      { id: 'complaintRate',     label: 'Complaint / Callback Rate',   placeholder: 'e.g. 3.2 per 1,000', internal: 'Cleanliness complaints or callbacks per relevant unit of volume',
        defaultValue: '3.2 per 1,000 room-nights', defaultTranslation: '' },
      { id: 'supplyCostImpact',  label: 'Supply Cost Impact',          placeholder: 'e.g. +7.5% chemical/paper', internal: 'Market-driven supply cost increases outside your control',
        defaultValue: '+7.5% chemical & paper (national index)', defaultTranslation: 'Cleaning chemical and paper costs are up 7.5% nationally per industry index data — a market condition affecting every provider, not specific to how we are running this account.' },
    ],
    financialChallenges: [
      "Your inspection score dropped five points and now you're telling me you need more staff? Help me understand that.",
      "Turnover time is 12 minutes over target during our busiest week. That's exactly when it matters most — what's driving it?",
      "Complaints are trending up again. Should I be worried about how this reflects on us during the next survey or tour?",
      "I'm looking at a labor line that's up nearly two points. Walk me through what's actually driving that.",
    ],
    qbrSections: [
      { id: 'executiveSummary', label: 'Executive Summary', desc: 'Lead with the headline — what is the one thing leadership should take away from this QBR?',
        defaultValue: `Q2 inspection scores dipped to 88%, down 5 points, concentrated in a single unit during a short-staffed stretch we have since corrected. Turnover time is running above target during peak volume, and we have a staffing change going live next week to address it. One item — a supply cost pass-through — needs your direction before Fall.` },
      { id: 'financialPerformance', label: 'Financial & Service Performance', desc: 'Inspection scores, turnover time, labor — in client language, not operator language.',
        defaultValue: `Inspection scores came in at 88%, down 5 percentage points from last period. Turnover time averaged 42 minutes against a 30-minute target during peak-volume shifts. Labor is running at 36.4% of budget, including 5.1 points of overtime covering two open positions, one of which is filled starting in two weeks. Supply costs are up 7.5% nationally on chemical and paper products — a market condition, not an operational variance.` },
      { id: 'experienceSection', label: 'Occupant Impact & Compliance', desc: 'What occupants and inspectors experienced, what it means, and what you are doing about it.',
        defaultValue: `Complaint volume held at 3.2 per 1,000, in line with the prior period. The inspection score decline did not trigger any compliance or regulatory concern, and we have already corrected the documentation gap that accounted for two of the five points lost. We proactively briefed the affected department lead before the next scheduled walkthrough.` },
      { id: 'strategicInitiatives', label: 'Strategic Initiatives', desc: 'What are you building or improving that creates value beyond the contract scope?',
        defaultValue: '' },
      { id: 'lookingAhead', label: 'Looking Ahead', desc: 'What decisions do you need from leadership in the next 30-60 days?',
        defaultValue: `Two items need your direction before Fall. First, the 7.5% supply cost increase — we have modeled the pass-through impact and need your alignment on timing before we adjust next quarter's invoice. Second, we would like your sign-off on a staggered staffing model to bring peak-shift turnover time back under target ahead of the busiest season at the ${base.siteTerm}.` },
      ],
    qbrPersonas: [
      { id: 'cfo', label: `${base.decisionMakerTitle} / Finance`, icon: '💼', style: `Focused on labor cost and contract performance. Will probe overtime spend and any proposed cost pass-throughs. Will NOT ask about the provider's internal margins. Expects the GM to speak in client dollars and outcomes, not shift-level operating detail.` },
      { id: 'facilities_liaison', label: 'Director of Facilities / Environmental Services', icon: '🧹', style: `Cares about inspection readiness, turnover time, and complaint trends. Will push hard on any inspection score decline and ask for a specific corrective plan with a date attached.` },
      { id: 'exec_sponsor', label: 'Executive Sponsor', icon: '🏛️', style: `Focused on reputation and appearance — wants the headline story on inspection and audit readiness. Expects the GM to speak like a strategic partner, not a vendor reading a checklist.` },
    ],
    diagnosticQuestions: [
      { id: 'financial',    label: `Explaining an inspection score decline or cost pass-through to a ${base.decisionMakerTitle} in client language` },
      { id: 'challenge',    label: `Responding to a ${base.decisionMakerTitle} who questions a staffing or scope expansion request` },
      { id: 'contract',     label: 'Reading and understanding your own contract, including scope and staffing commitments' },
      { id: 'stakeholder',  label: `Identifying who informally influences housekeeping decisions at your ${base.siteTerm}` },
      { id: 'executive',    label: 'Delivering a structured update to an executive under pressure' },
      { id: 'recovery',     label: 'Having a direct conversation about a service failure or compliance/inspection issue' },
      { id: 'qbr',          label: 'Presenting a QBR to a mixed executive audience' },
      { id: 'scope',        label: 'Saying "let me price that" when a client asks for more service' },
      { id: 'retention',    label: 'Seeing client retention and compliance risk signals early enough to respond' },
      { id: 'relationships',label: 'Building genuine relationships above your primary contact' },
...
// trainingType is per-industry (see TRAINING_TYPES) and includes both the
// industry's dining/core track (e.g. 'executive-communication') and the two
// shared facilities tracks ('facilities-maintenance' / 'facilities-housekeeping').
// getSelectedServiceLine reduces that down to just the facilities dimension,
// since that's the only split that currently changes stakeholder/financial/QBR
// content. Returns null for dining/core tracks so callers fall back to the
// industry's own config.
export function getSelectedServiceLine() {
  if (typeof window === 'undefined') return null
  const tt = sessionStorage.getItem('selectedTrainingType')
  return (tt === 'facilities-maintenance' || tt === 'facilities-housekeeping') ? tt : null
}

// getIndustryConfig(industryId, serviceLine) — serviceLine is optional so every
// existing call site (which only ever passed industryId) keeps working exactly
// as before and gets the dining-flavored config. Pass a serviceLine to layer
// the facilities override on top of the industry's base account/site fields.
export function getIndustryConfig(industryId, serviceLine) {
  const base = INDUSTRY_CONFIG[industryId] || INDUSTRY_CONFIG['higher-ed']
  const overrideFn = serviceLine && SERVICE_LINE_CONFIG[serviceLine]
  if (!overrideFn) return base
  return { ...base, ...overrideFn(base), serviceLine }
}

// Layers an org's own approved custom content (generated from their
// uploaded policies — see lib/policies.js, /api/org-config) on top of the
// resolved industry+serviceLine config. orgConfig is either null (no
// approved override exists — cfg is returned unchanged) or the raw `config`
// jsonb from org_module_config, shaped as:
//   financial -> { financialMetrics, financialChallenges }
//   qbr       -> { qbrSections, qbrPersonas }
export function applyOrgConfig(cfg, orgConfig) {
  if (!orgConfig) return cfg
  return { ...cfg, ...orgConfig }
}
