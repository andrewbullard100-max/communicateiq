...
Generate exactly 3 scenarios as a JSON array. Each element must have this exact shape:
{
...
Output ONLY the JSON array, no preamble, no markdown fences.`
  ...
  const userPrompt = `ORGANIZATION'S POLICY DOCUMENTS:\n${policyContext}\n\nIndustry: ${base.label || industryId}\nTraining track: ${trainingType}\n\nGenerate 3 scenarios for this training track grounded in the policies above.`

  const scenarios = await callGenerationModel(system, userPrompt, 4000)
  ...
  for (const s of scenarios) {
    const { data: family, error: famErr } = await db
      ...
      .insert({ org_id: orgId, slug: `org-${orgId.slice(0, 8)}-${Date.now()}-${inserted.length}`, industry_id: industryId, training_type: trainingType })
      .select()
      .single()
    ...
    await db.from('scenario_families').update({ current_version_id: version.id }).eq('id', family.id)
    inserted.push({ familyId: family.id, versionId: version.id, title: s.title })
  }
  return inserted
}
