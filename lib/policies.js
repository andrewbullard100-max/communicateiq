...
export async function deletePolicyDocument(orgId, docId, { cascade = false } = {}) {
  const db = getSupabaseScoped(orgId)
  ...
  if (cascade) {
    const { error: delContentErr } = await db.from('scenario_versions').delete().eq('source_document_id', docId)
    if (delContentErr) {
      // Still blocked even after cascading — e.g. one of these scenario
      // versions has real simulation_attempts against it, which we
      // deliberately don't cascade-delete (that's a trainee's real score
      // history, not disposable draft content).
      if (delContentErr.code === '23503') {
        throw new Error('Some of the generated content from this document has already been used in training attempts, so it can\'t be removed automatically. That content needs to be handled individually.')
      }
      throw new Error(delContentErr.message)
    }
  }
  ...
  if (delErr) {
    // 23503 = foreign key violation — this document already has generated
    // scenarios/config (scenario_versions.source_document_id) pointing at
    // it, possibly already reviewed/approved and in front of trainees.
    // Surface a clear reason (and count) instead of the raw constraint
    // error, so the caller can offer a cascade-delete retry.
    if (delErr.code === '23503') {
      const { count } = await db
        .from('scenario_versions')
        .select('id', { count: 'exact', head: true })
        .eq('source_document_id', docId)
      const err = new Error('This document has generated content (scenarios, rubrics, or config) linked to it, so it can\'t be deleted on its own.')
      err.linkedCount = count || 1
      throw err
    }
...
export async function listDraftContent(orgId) {
  ...
  // Two plain queries + an in-JS join, rather than a single embedded-filter
  // query (scenario_families!inner(...) combined with
  // .eq('scenario_families.org_id', ...)) — that pattern is finicky enough
  // across PostgREST/postgrest-js versions that it's not worth the risk of
  // it silently returning zero rows. A direct .eq('org_id', orgId) on
  // scenario_families, then a plain .in('family_id', ids) on
  // scenario_versions, is unambiguous and easy to verify independently.
  const { data: families, error: famErr } = await db
    .from('scenario_families')
    .select('id, industry_id, training_type')
    .eq('org_id', orgId)
  if (famErr) throw new Error(famErr.message)

  const familyById = Object.fromEntries((families || []).map(f => [f.id, f]))
  const familyIds = Object.keys(familyById)

  const { data: scenarioRows, error: scErr } = familyIds.length
    ? await db
        .from('scenario_versions')
        .select('id, title, status, difficulty, day_label, created_at, source_document_id, family_id')
  ...
  return {
    scenarios: (scenarioRows || []).map(r => ({
      ...
      industryId: familyById[r.family_id]?.industry_id, trainingType: familyById[r.family_id]?.training_type,
