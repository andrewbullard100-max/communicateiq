...
export async function getUserAttempts(userId, orgId) {
  ...
  const { data, error } = await db
    ...
    .select('id, scenario_label, industry, training_type, ai_scores, ai_certification_status, ai_headline, pressure_level, completed_at, created_at')
    .eq('user_id', userId)
...
export async function getOrgAttempts(orgId, userId) {
  ...
  const { data, error } = await db
    .from('simulation_attempts')
    .select(
      'id, scenario_label, industry, training_type, ai_scores, ai_certification_status, ai_headline, pressure_level, completed_at, created_at, users!inner(email, name, org_id)'
    )
